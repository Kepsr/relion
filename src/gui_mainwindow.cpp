            ...
            if (exists(fn)) {
                if (fn_file == "run.out") {
                    if (maingui_do_read_only) {
                        ...
                        system("awk -F\"\r\" '{if (NF>1) {print $NF} else {print}}' < " + fn + " > .gui_tmpstd".c_str());
...
void SchedulerWindow::cb_execute_i() {
    ...
    if (exists(fn_check)) {
        ...
        std::string command = "relion_pipeliner"
...
GuiMainWindow::GuiMainWindow(
    ...
    if (exists(pipeline.name + "_pipeline.star")) {
        pipeline.read(DO_LOCK, "mainGUI constructor");
    ...
    if (!maingui_do_read_only) {
        ...
        for (int i = 0; i < schedules.size(); i++) {
            menubar->add(("Schedules/" + schedules[i]).c_str(), 0, cb_toggle_schedule, this);
        }
...
std::string GuiMainWindow::getJobNameForDisplay(Process &job) {
    ...
    if (show_scheduler) {
        return job.name.afterFirstOf(schedule.name).beforeLastOf("/");
    ...
    if (do_order_alphabetically) {
        ...
        for (long int ip = 0; ip < enumerate_jobs.size(); ip++) {
            ...
            if (
                ...
                finished_job_browser->add(decorate_name_or_alias(vp[ip].first, pipeline.processList[i].status).c_str());
            }
        }
    } else {
    ...
    if (current_job >= 0) {
        ...
        for (long int inode = 0; inode < pipeline.processList[current_job].inputNodeList.size(); inode++) {
            ...
            if (myproc >= 0) {
                ...
                if (!already_there(myproc, input_processes)) {
                    input_processes.push_back(myproc);
                    std::string jobname = getJobNameForDisplay(pipeline.processList[myproc]);
                    if (show_scheduler) {
        ...
        for (long int inode = 0; inode < pipeline.processList[current_job].outputNodeList.size(); inode++) {
            ...
            for (long int iproc = 0; iproc < nr_outputs; iproc++) {
                ...
                // Check if this process was already there
                if (!already_there(myproc, output_processes)) {
...
bool already_there(long int proc, std::vector<long int> procs) {
...
void GuiMainWindow::fillSchedulerNodesAndVariables() {
    ...
    for (int i = 0; i < schedule.edges.size(); i++) {
        std::string mylabel;
        mylabel = schedule.edges[i].is_fork ?
            schedule.edges[i].inputNode + " -> (" + schedule.edges[i].myBooleanVariable + ") ? " + schedule.edges[i].outputNodeTrue + " : " + schedule.edges[i].outputNode :
            schedule.edges[i].inputNode + " -> " + schedule.edges[i].outputNode;
...
std::string scheduler_get_io(auto scheduler_edge_io, std::string io) {
...
void GuiMainWindow::cb_add_scheduler_edge_i() {
    ...
    if (scheduler_edge_boolean->value() >= 0) {
        // Get outputtrue
        std::string outputtrue = scheduler_get_io(scheduler_edge_outputtrue, "outputtrue");
        if (input == "") return;
        schedule.read(DO_LOCK);
        schedule.addFork(input, scheduler_edge_boolean->text(i), scheduler_edge_outputtrue->text(j), output);
...
void GuiMainWindow::cb_delete_scheduler_edge_i() {
    if (scheduler_edge_browser->value() <= 0) {
...
void GuiMainWindow::cb_select_scheduler_edge_i() {
    ...
    if ((int i = scheduler_edge_browser->value()) >= 1) {
...
bool user_wants_to(std::string action, std::string describe_action) {
    return fl_choice("%s", "Cancel", action, NULL, "Are you sure you want to " + describe_action + "?".c_str())
}
...
void GuiMainWindow::cb_display_i() {
    system(" relion_display --gui &".c_str());
}
...
void GuiMainWindow::cb_delete_scheduler_job_i() {
    ...
    for (size_t i = 0; i < deleteProcesses.size(); i++) {
        if (deleteProcesses[i])
            describe_action += " - " + getJobNameForDisplay(pipeline.processList[i]) + "\n";
    }
    if (user_wants_to("Move", describe_action)) {
        ...
        for (int i = 0; i < deleteProcesses.size(); i++)
            if (deleteProcesses[i])
                schedule.removeJob(getJobNameForDisplay(pipeline.processList[i]));
        schedule.write(DO_LOCK);
...
void GuiMainWindow::cb_delete_i(bool ask, bool recursively) {
    if (current_job < 0) {
        fl_message("Please select a job.");
        return;
    }
    ...
    if (ask) {
        ...
        for (size_t i = 0; i < deleteProcesses.size(); i++) {
            if (deleteProcesses[i]) {
                auto job = pipeline.processList[i];
...
void GuiMainWindow::cb_cleanup_i(int jobindex, bool ask, bool harshly) {
    ...
    if (!ask || user_wants_to("Clean up", "clean up intermediate files from " + pipeline.processList[current_job].name)) {
        std::string error_message;
        if (!pipeline.cleanupJob(myjob, harshly, error_message))
            fl_message("%s", error_message.c_str());
    }
}
...
void GuiMainWindow::cb_edit_note_i(bool is_project_note) {
    ...
    if (is_project_note) {
        ...
        auto job = pipelin.processList[current_job];
...
void GuiMainWindow::cb_export_jobs_i() {
    ...
    const char* answer = fl_input("Name of the exported block of jobs? ", "export1".c_str());
...
void GuiMainWindow::cb_create_schedule(Fl_Widget* o, void* v) {
    ...
    const char* name = fl_input("%s", "", " Name of the new schedule: ".c_str());
...
void GuiMainWindow::cb_copy_schedule(Fl_Widget* o, void* v) {
    ...
    FileName fn_copy = std::string(fl_input(
        "%s", "", " Name of the copy schedule: ".c_str()
    ));
...
void GuiMainWindow::cb_start_pipeliner_i() {
    ...
    for (long int i = 0; i < scheduled_processes.size(); i++) {
        long int id = scheduled_processes[i];
...
void GuiMainWindow::cb_stop_pipeliner_i() {
    Fl_File_Chooser chooser(
        ".",
        "Pipeline scheduled file (RUNNING_PIPELINER_" + pipeline.name + "_*)".c_str(),
...
void GuiMainWindow::cb_toggle_expand_stdout_i(Fl_Widget* o, void* v) {
    GuiMainWindow* T = (GuiMainWindow*)v;
...
any redistribution of the code (and modified versions thereof) is appreciated (contact Sjors directly).\n\n\
