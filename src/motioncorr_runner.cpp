...
void MotioncorrRunner::initialise() {
    ...
    if (!fn_in.isStarFile() && voltage < 0.0) {
    ...
    for (long int imic = 0; imic < fn_mic_given_all.size(); imic++) {
        ...
        if (continue_old) {
            ...
            if (
                exists(fn_avg) && exists(fn_avg.withoutExtension() + ".star") && (
...
bool MotioncorrRunner::executeMotioncor2(Micrograph &mic, int rank) {
    ...
    command += fn_mic.getExtension() == "tif" || fn_mic.getExtension() == "tiff" ?
        " -InTiff " + fn_mic :
        " -InMrc " + fn_mic;
...
void MotioncorrRunner::plotShifts(FileName fn_mic, Micrograph &mic) {
    ...
    for (int j = mic.first_frame, jlim = mic.getNframes(); j <= jlim; j++) {
        // 1-indexed
...
bool MotioncorrRunner::executeOwnMotionCorrection(Micrograph &mic) {
    ...
    // Debug output
    for (int iframe = 0; iframe < n_frames; iframe++) {
        Iframes[iframe].write(
            fn_avg.withoutExtension() + "_frames.mrcs", iframe, 
        ...
        for (int iy = 0; iy < patch_y; iy++) {
            for (int ix = 0; ix < patch_x; ix++) {
                ...
                interpolateShifts(
                    ...
                    local_xshifts, local_yshifts, n_frames, 
                    interpolated_xshifts, interpolated_yshifts
                );
...
void MotioncorrRunner::interpolateShifts(
    std::vector<int> &group_start, std::vector<int> &group_size,
    ...
    int cur_group = 0;
    for (int iframe = 0; iframe < n_frames; iframe++) {
        if (cur_group < n_groups - 2 && iframe >= centers[cur_group + 1]) {
            cur_group++; // don't go to the last group
        }
        // This formula can be used for both interpolation and extrapolation
        interpolated_xshifts[iframe] = (
            xshifts[cur_group    ] * (centers[cur_group + 1] - iframe)
          + xshifts[cur_group + 1] * (iframe - centers[cur_group    ])
        ) / (centers[cur_group + 1] - centers[cur_group]);
        interpolated_yshifts[iframe] = (
            yshifts[cur_group] * (centers[cur_group + 1] - iframe)
...
void MotioncorrRunner::realSpaceInterpolation(
    Image<float> &Isum, std::vector<Image<float>> &Iframes, MotionModel *model, 
    std::ostream &logfile
) {
...
void MotioncorrRunner::realSpaceInterpolation_ThirdOrderPolynomial(
    Image <float> &Isum, std::vector<Image<float> > &Iframes, 
...
bool MotioncorrRunner::alignPatch(
    std::vector<MultidimArray<fComplex>> &Fframes, 
    const int pnx, const int pny, const RFLOAT scaled_B, 
    std::vector<RFLOAT> &xshifts, std::vector<RFLOAT> &yshifts, 
    std::ostream &logfile
) {
    ...
    search_range /= std::max(ccf_scale_x, ccf_scale_y); // account for the increase of pixel size in CCF
...
int MotioncorrRunner::findGoodSize(int request) {
    ...
    // Return least element n of good_numbers such that n >= request
    for (int i = 0; good_numbers[i] != -1; i++) {
        if (good_numbers[i] >= request) {
            return good_numbers[i];
        }
    }
    // Couldn't find an appropriate element of good_numbers
    return request;
}
...
bool MotioncorrRunner::detectSerialEMDefectText(FileName fn_defect) {
    ...
    while (std::getline(f_defect, line)) {
        if (
            line.find("CameraSize")      != std::string::npos ||
