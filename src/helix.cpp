...
void makeHelicalSymmetryList(
    ...
    for (int ii = 0; ii < list.size(); ii++) {
        for (int jj = 0; jj < tmp_list.size(); jj++) {
            ...
            twist_avg_deg = (fabs(list[ii].twist_deg) + fabs(tmp_list[jj].twist_deg)) / 2.0;
            rise_avg_pix  = (fabs(list[ii].rise_pix) + fabs(tmp_list[jj].rise_pix)) / 2.0;
...
bool calcCCofHelicalSymmetry(
    ...
    double dist_r_pix, sum_pw1, sum_pw2, sum_n, sum_chunk = 0., sum_chunk_n = 0.0;
    ...
    FOR_ALL_ELEMENTS_IN_ARRAY3D(v, i, j, k) {
        ...
        // dev_voxel.clear();
        // dev_voxel.push_back(v.elem(i, j, k));

        // Pick other voxels according to this voxel and helical symmetry
        RFLOAT zp = k;
        int rot_id = 0;
        sum_pw1 = v.elem(i, j, k);
        sum_pw2 = v.elem(i, j, k) * v.elem(i, j, k);
        sum_n = 1.0;
        while (true) {
            // Rise
            zp += rise_pix;
...
bool localSearchHelicalSymmetry(
    ...
    rise_refined_A = (rise_min_A + rise_max_A) / 2.0;
    twist_refined_deg = (twist_min_deg + twist_max_deg) / 2.0;
...
RFLOAT getHelicalSigma2Rot(
    ...
    RFLOAT new_rot_step = rot_search_range / 6.0;
...
bool checkParametersFor3DHelicalReconstruction(
    ...
    RFLOAT nr_units_min = 2.0; // Minimum nr_particles required along lenZ_max
    ...
    RFLOAT rise_avg_A = (rise_min_A + rise_max_A) / 2.0;
    RFLOAT twist_avg_deg = (twist_min_deg + twist_max_deg) / 2.0;
    ...
    RFLOAT z_percentage_max = ( (2.0) * sqrt( (particle_diameter_A * particle_diameter_A / 4.0) - (tube_outer_diameter_A * tube_outer_diameter_A / 4.0) ) / pixel_size_A) / ((RFLOAT)(box_len));
...
void imposeHelicalSymmetryInRealSpace(
    ...
    FOR_ALL_ELEMENTS_IN_ARRAY3D(v, i, j, k) {
        ...
        if (pix_weight > 0.9) {
            vout.elem(i, j, k) = pix_sum / pix_weight;
        ...
        std::ofstream* fout_ptr)
{
    ...
    if ( (r_max_pix < 2.0) || (r_min_pix < 0.0) || ((r_max_pix - r_min_pix) < 2.0)
        ...
        RFLOAT cyl_outer_radius_A)
{
    ...
    if ( (sphere_radius_pix < 2.0) || (sphere_radius_pix > half_box_len)
            || (cyl_radius_pix < 2.0) || (cyl_radius_pix > half_box_len)
        ...
        RFLOAT search_step_deg)
{
    ...
    if ( (search_step_deg < 0.000001) || (search_step_deg > 2.0) )
...
void cutZCentralPartOfSoftMask(
    ...
    idz_e = (RFLOAT) Zdim * z_percentage / 2.0;
    idz_s = idz_e * -1.0;
...
void convertHelicalTubeCoordsToMetaDataTable(
    ...
    int nr_segments;
    RFLOAT psi_deg, psi_rad, x1, y1, x2, y2, dx, dy, step_pix, half_box_size_pix, len_pix, psi_prior_flip_ratio;
    MetaDataTable MD_in;
    ...
    for (int tube_id = 0; tube_id < x1_coord_list.size(); tube_id++) {
        ...
        RFLOAT xp = x1 - (dx * 0.99);
        RFLOAT yp = y1 - (dy * 0.99);
        len_pix = -step_pix;
        while (1) {
            xp += dx;
            yp += dy;
...
void HelicalSegmentPriorInfoEntry::clear() {
    ...
    rot_prior_deg = psi_prior_deg = tilt_prior_deg = 0.0;  // KThurber
    dx_prior_A = dy_prior_A = dz_prior_A = 0.0;
    psi_flip_ratio = 0.0;
        ...
        for (int id = sid; id <= eid; id++) {
            ...
            if (do_avg) {
                ...
                if (offset2 > range2_offset) {
                    ...
                    transformCartesianAndHelicalCoords(sum_trans_vec, sum_trans_vec, (is_3D_data) ? (this_rot) : (0.0), (is_3D_data) ? (this_tilt) : (0.0), this_psi, HELICAL_TO_CART_COORDS); // Averaged translations - use respective averaged tilt and psi
...
void updateAngularPriorsForHelicalReconstructionFromLastIter(
    ...
    for (long int index : MD) {
        if (have_tilt && have_tilt_prior && !keep_tilt_prior_fixed) {
            MD.setValue(EMDL::ORIENT_TILT_PRIOR, MD.getValue<RFLOAT>(EMDL::ORIENT_TILT));
        }
        if (have_psi && have_psi_prior) {
            MD.setValue(EMDL::ORIENT_PSI_PRIOR, MD.getValue<RFLOAT>(EMDL::ORIENT_PSI));
        }
        // KThurber add rot section
        if (have_rot && have_rot_prior) {
            MD.setValue(EMDL::ORIENT_ROT_PRIOR, MD.getValue<RFLOAT>(EMDL::ORIENT_ROT));
        }
    }
}
...
void HermiteInterpolateOne3DHelicalFilament(
    ...
    for (int id = 0; id < xlist.size() - 1; id++) {
        ...
        //step_pix = (interbox_pix < 5.7735) ? (0.57735) : (interbox_pix / 10.0); // 10%
