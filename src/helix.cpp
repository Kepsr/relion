...
bool localSearchHelicalSymmetry(
    const MultidimArray<RFLOAT>& v,
    RFLOAT pixel_size_A, RFLOAT sphere_radius_A,
    RFLOAT cyl_inner_radius_A, RFLOAT cyl_outer_radius_A, RFLOAT z_percentage,
    RFLOAT rise_min_A,    RFLOAT rise_max_A,    RFLOAT rise_inistep_A,    RFLOAT& rise_refined_A,
    RFLOAT twist_min_deg, RFLOAT twist_max_deg, RFLOAT twist_inistep_deg, RFLOAT& twist_refined_deg,
    std::ostream* o_ptr
) {
    ...
    checkParametersFor3DHelicalReconstruction(
        false, true, 1,
        rise_refined_A,    rise_min_A,    rise_max_A,
        twist_refined_deg, twist_min_deg, twist_max_deg,
        box_len,
        pixel_size_A, z_percentage,
        sphere_radius_A * 2.0, cyl_inner_radius_A * 2.0, cyl_outer_radius_A * 2.0
    );
    ...
    for (iter = 1; iter <= 100; iter++) {
        ...
        makeHelicalSymmetryList(
            ...
            rise_local_min_pix,  rise_local_max_pix,  rise_step_pix,  search_rise,
            twist_local_min_deg, twist_local_max_deg, twist_step_deg, search_twist
        );
        ...
        for (int ii = 0; ii < helical_symmetry_list.size(); ii++) {
            ...
            if (helical_symmetry_list[ii].dev > 1e30) {
                ...
                calcCCofHelicalSymmetry(
                    v, r_min_pix, r_max_pix, z_percentage,
        ...
        if (iter > 1 && !out_of_range) {
            ...
            if (search_rise) {
                if (
                    rise_refined_pix < rise_local_min_pix + rise_step_pix * 0.5 ||
                    rise_refined_pix > rise_local_max_pix - rise_step_pix * 0.5
                ) {
                    this_iter_not_converged = true;
                    rise_local_min_pix = rise_refined_pix - ((RFLOAT) iter_not_converged + 3.0) * rise_step_pix;
                    rise_local_max_pix = rise_refined_pix + ((RFLOAT) iter_not_converged + 3.0) * rise_step_pix;
                }
            }
            if (search_twist) {
                if (
                    twist_refined_deg < twist_local_min_deg + twist_step_deg * 0.5 ||
                    twist_refined_deg > twist_local_max_deg - twist_step_deg * 0.5
                ) {
                    this_iter_not_converged = true;
                    twist_local_min_deg = twist_refined_deg - ((RFLOAT) iter_not_converged + 3.0) * twist_step_deg;
                    twist_local_max_deg = twist_refined_deg + ((RFLOAT) iter_not_converged + 3.0) * twist_step_deg;
                }
            }
            if (this_iter_not_converged) {
                ...
                if (iter_not_converged > 10) {
                    // Up to 25*25 samplings are allowed (original 5*5 samplings)
                    if (o_ptr != NULL)
...
RFLOAT getHelicalSigma2Rot(
    ...
    RFLOAT helical_offset_step_Angst, RFLOAT rot_step_deg,
    RFLOAT old_sigma2_rot
) {
    if (
        helical_offset_step_Angst < 0.0 ||
        rot_step_deg              < 0.0 ||
        old_sigma2_rot            < 0.0
    ) {
...
bool checkParametersFor3DHelicalReconstruction(
    ...
    int nr_asu,
    RFLOAT rise_initial_A,    RFLOAT rise_min_A,    RFLOAT rise_max_A,
    RFLOAT twist_initial_deg, RFLOAT twist_min_deg, RFLOAT twist_max_deg,
    int box_len,
    RFLOAT pixel_size_A, RFLOAT z_percentage, RFLOAT particle_diameter_A,
    RFLOAT tube_inner_diameter_A, RFLOAT tube_outer_diameter_A,
    bool verboseOutput
) {
    ...
    if (particle_radius_pix < 2.0 || particle_radius_pix > half_box_len) {
        if (verboseOutput) {
    ...
    if (verboseOutput) {
        if (tube_inner_diameter_A > 0.0)
            std::cout << " Inner tube diameter = " << tube_inner_radius_pix * 2.0 << " pixels = " << tube_inner_diameter_A << " Angstroms" << std::endl;
            std::cout << " Outer tube diameter = " << tube_outer_radius_pix * 2.0 << " pixels = " << tube_outer_diameter_A << " Angstroms" << std::endl;
    }
    if (
        tube_outer_radius_pix < 2.0 ||
        tube_outer_radius_pix > half_box_len ||
    ...
    if (tube_inner_radius_pix > 0.0 && tube_inner_radius_pix + 2.0 > tube_outer_radius_pix) {
        if (verboseOutput)
    ...
    if (
        fabs(twist_min_deg)     > 360.0 ||
        fabs(twist_initial_deg) > 360.0 ||
        fabs(twist_max_deg)     > 360.0
    ) {
        if (verboseOutput)
    ...
    if (
        rise_min_A     < 0.001 || rise_min_A / pixel_size_A     < 0.001 ||
    ...
    if (do_symmetry_local_refinement) {
        if (
            twist_min_deg > twist_max_deg || twist_initial_deg < twist_min_deg || twist_initial_deg > twist_max_deg ||
    ...
    if (do_symmetry_local_refinement) {
        ...
        if (fabs(rise_max_A) > rise_max_upper_bound_A) {
            // THIS CANNOT HAPPEN. ERRORS HAVE ALREADY BEEN RAISED IN Z PERCENTAGE CHECK.
            if (verboseOutput)
    ...
    RFLOAT half_nr_asu_max = 0.5 * (1.0 - z_percentage) * (RFLOAT(box_len)) * pixel_size_A / rise_max_A;
...
void imposeHelicalSymmetryInRealSpace(
    ...
    checkParametersFor3DHelicalReconstruction(
        false, false, 1,
        rise_A,    rise_A,    rise_A,
        twist_deg, twist_deg, twist_deg,
        box_len, pixel_size_A, z_percentage,
        sphere_radius_A * 2.0, cyl_inner_radius_A * 2.0, cyl_outer_radius_A * 2.0
    );

    // Parameters of mask
    if (cosine_width_pix < 1.0)
        cosine_width_pix = 1.0;  // Avoid division by 0
...
void calcRadialAverage(
    ...
    for (ii = 0; ii < list_size; ii++) {
        radial_pix_counter_list[ii] = 0.0;
...
void createCylindricalReference(
    MultidimArray<RFLOAT>& v, int box_size,
    RFLOAT inner_diameter_pix, RFLOAT outer_diameter_pix, RFLOAT cosine_width
) {
    ...
    if (
        inner_diameter_pix > outer_diameter_pix ||
        outer_diameter_pix < 0.0 ||
        outer_diameter_pix > box_size - 1 ||
        cosine_width < 0.0
    ) {
        REPORT_ERROR("helix.cpp::createCylindricalReference(): Parameter(s) error!");
    }

    inner_radius_pix = inner_diameter_pix / 2.0;
    outer_radius_pix = outer_diameter_pix / 2.0;
...
void createCylindricalReferenceWithPolarity(
    ...
    if (
        inner_diameter_pix > outer_diameter_pix || outer_diameter_pix < 0.0 ||
        outer_diameter_pix > box_size - 1 ||
        ratio_topbottom < 0.0 ||  ratio_topbottom > 1.0 ||
        cosine_width < 0.0
    ) {
    ...
    top_radius_pix = outer_diameter_pix / 2.0;
    bottom_radius_pix = outer_diameter_pix * ratio_topbottom / 2.0;
    if (inner_diameter_pix > 0.0)
        bottom_radius_pix = (inner_diameter_pix + ratio_topbottom * (outer_diameter_pix - inner_diameter_pix)) / 2.0;
...
void applySoftSphericalMask(
    ...
    //r_max *= 0.45;
    if (sphere_diameter > 0.01 && sphere_diameter / 2.0 < r_max)
...
void extractHelicalSegmentsFromTubes_Multiple(
    FileName& suffix_in, FileName& suffix_out,
    int format_tag, int nr_asu,
    RFLOAT rise_A, RFLOAT pixel_size_A,
    RFLOAT Xdim, RFLOAT Ydim,
    RFLOAT box_size_pix,
    ...
    for (int ii = 0; ii < fn_in_list.size(); ii++) {
        ...
        if (format_tag == RELION_STAR_FORMAT) {
            convertHelicalTubeCoordsToMetaDataTable(
                fn_in_list[ii], MD_out, nr_segments, nr_tubes, nr_asu,
                rise_A, pixel_size_A, Xdim, Ydim, box_size_pix,
                bimodal_angular_priors, cut_into_segments
            );
        } else if (format_tag == XIMDISP_COORDS_FORMAT) {
            convertXimdispHelicalTubeCoordsToMetaDataTable(
                fn_in_list[ii], MD_out, nr_segments, nr_tubes, nr_asu,
                rise_A, pixel_size_A, Xdim, Ydim, box_size_pix,
                bimodal_angular_priors, cut_into_segments
            );
...
void convertHelicalTubeCoordsToMetaDataTable(
    FileName& fn_in, MetaDataTable& MD_out,
    int& total_segments, int& total_tubes, int nr_asu,
    RFLOAT rise_A, RFLOAT pixel_size_A,
    RFLOAT Xdim, RFLOAT Ydim, RFLOAT box_size_pix,
    ...
    half_box_size_pix = box_size_pix / 2.0;
    ...
    bool MDin_has_id = MD_in.containsLabel(EMDL::PARTICLE_HELICAL_TUBE_ID);
    bool MDin_has_pitch =  MD_in.containsLabel(EMDL::PARTICLE_HELICAL_TUBE_PITCH);
    x1_coord_list.clear();
    y1_coord_list.clear();
    x2_coord_list.clear();
    y2_coord_list.clear();
    ...
    FOR_ALL_OBJECTS_IN_METADATA_TABLE(MD_in) {
        MDobj_id++;
        MD_in.getValue(EMDL::IMAGE_COORD_X, xp);
        MD_in.getValue(EMDL::IMAGE_COORD_Y, yp);
        if (MDobj_id % 2) {
            x1_coord_list.push_back(xp);
            y1_coord_list.push_back(yp);
            if (MDin_has_id) {
                MD_in.getValue(EMDL::PARTICLE_HELICAL_TUBE_ID, id);
                tube_id_list.push_back(id);
            }
            if (MDin_has_pitch) {
                MD_in.getValue(EMDL::PARTICLE_HELICAL_TUBE_PITCH, pitch);
    ...
    if (
        x1_coord_list.size() != x2_coord_list.size() ||
    ...
    MD_out.clear();
    MD_out.addLabel(EMDL::IMAGE_COORD_X);
    MD_out.addLabel(EMDL::IMAGE_COORD_Y);
    MD_out.addLabel(EMDL::PARTICLE_HELICAL_TUBE_ID);
    MD_out.addLabel(EMDL::ORIENT_TILT_PRIOR);
    MD_out.addLabel(EMDL::ORIENT_PSI_PRIOR);
    MD_out.addLabel(EMDL::PARTICLE_HELICAL_TRACK_LENGTH_ANGSTROM);
    MD_out.addLabel(EMDL::ORIENT_PSI_PRIOR_FLIP_RATIO);

    if (MDin_has_id)
        MD_out.addLabel(EMDL::PARTICLE_HELICAL_TUBE_ID);
    if (MDin_has_pitch)
        MD_out.addLabel(EMDL::PARTICLE_HELICAL_TUBE_PITCH);
    ...
    for (int tube_id = 0; tube_id < x1_coord_list.size(); tube_id++) {
        ...
        if (!cut_into_segments) {
            MD_out.addObject();
            MD_out.setValue(EMDL::IMAGE_COORD_X, ((x1 + x2) / 2.0));
            MD_out.setValue(EMDL::IMAGE_COORD_Y, ((y1 + y2) / 2.0));
            MD_out.setValue(EMDL::PARTICLE_HELICAL_TUBE_ID, (tube_id + 1));
            MD_out.setValue(EMDL::ORIENT_TILT_PRIOR, 90.0);
            MD_out.setValue(EMDL::ORIENT_PSI_PRIOR, -psi_deg);
            MD_out.setValue(EMDL::PARTICLE_HELICAL_TRACK_LENGTH_ANGSTROM, 0.0);
            MD_out.setValue(EMDL::ORIENT_PSI_PRIOR_FLIP_RATIO, psi_prior_flip_ratio);

            if (MDin_has_id)
                MD_out.setValue(EMDL::PARTICLE_HELICAL_TUBE_ID, id);
            if (MDin_has_pitch)
                MD_out.setValue(EMDL::PARTICLE_HELICAL_TUBE_PITCH, pitch);
        ...
        while (1) {
            ...
            if (
                ...
                if (
                    xp < half_box_size_pix || xp > Xdim - half_box_size_pix ||
                ...
                MD_out.addObject();
                MD_out.setValue(EMDL::IMAGE_COORD_X, xp);
                MD_out.setValue(EMDL::IMAGE_COORD_Y, yp);
                MD_out.setValue(EMDL::PARTICLE_HELICAL_TUBE_ID, (tube_id + 1));
                MD_out.setValue(EMDL::ORIENT_TILT_PRIOR, 90.);
                MD_out.setValue(EMDL::ORIENT_PSI_PRIOR, -psi_deg);
                MD_out.setValue(EMDL::PARTICLE_HELICAL_TRACK_LENGTH_ANGSTROM, pixel_size_A * len_pix);
                MD_out.setValue(EMDL::ORIENT_PSI_PRIOR_FLIP_RATIO, psi_prior_flip_ratio);
...
void combineParticlePriorsWithKaiLocalCTF(
    FileName& fn_priors, FileName& fn_local_ctf, FileName& fn_combined
) {
...
//		REPORT_ERROR("helix.cpp::combineParticlePriorsWithKaiLocalCTF(): File names must be different.");
    ...
    if (
        !MD_priors.containsLabel(EMDL::IMAGE_COORD_X) ||
        !MD_priors.containsLabel(EMDL::IMAGE_COORD_Y) ||
        // !MD_local_ctf.containsLabel(EMDL::MICROGRAPH_NAME) ||
        !MD_local_ctf.containsLabel(EMDL::IMAGE_COORD_X) ||
        !MD_local_ctf.containsLabel(EMDL::IMAGE_COORD_Y) ||
        !MD_local_ctf.containsLabel(EMDL::CTF_VOLTAGE) ||
        !MD_local_ctf.containsLabel(EMDL::CTF_DEFOCUSU) ||
        !MD_local_ctf.containsLabel(EMDL::CTF_DEFOCUSV) ||
        !MD_local_ctf.containsLabel(EMDL::CTF_DEFOCUS_ANGLE) ||
        !MD_local_ctf.containsLabel(EMDL::CTF_CS) ||
        !MD_local_ctf.containsLabel(EMDL::CTF_DETECTOR_PIXEL_SIZE) ||
        !MD_local_ctf.containsLabel(EMDL::CTF_MAGNIFICATION) ||
        !MD_local_ctf.containsLabel(EMDL::CTF_Q0)
    ...
    FOR_ALL_OBJECTS_IN_METADATA_TABLE(MD_local_ctf) {
        // This will look so much nicer when getValue returns the data, rather than a success/failure signal.
        MD_local_ctf.getValue(EMDL::IMAGE_COORD_X, _x);             x   .push_back(_x);
        MD_local_ctf.getValue(EMDL::IMAGE_COORD_Y, _y);             y   .push_back(_y);
        MD_local_ctf.getValue(EMDL::CTF_DEFOCUSU, _dU);             dU  .push_back(_dU);
        MD_local_ctf.getValue(EMDL::CTF_DEFOCUSV, _dV);             dV  .push_back(_dV);
        MD_local_ctf.getValue(EMDL::CTF_DEFOCUS_ANGLE, _dAng);      dAng.push_back(_dAng);
        MD_local_ctf.getValue(EMDL::CTF_CS, _Cs);                   Cs  .push_back(_Cs);
        MD_local_ctf.getValue(EMDL::CTF_DETECTOR_PIXEL_SIZE, _pix); pix .push_back(_pix);
        MD_local_ctf.getValue(EMDL::CTF_MAGNIFICATION, _mag);       mag .push_back(_mag);
        MD_local_ctf.getValue(EMDL::CTF_Q0, _Q0);                   Q0  .push_back(_Q0);
        MD_local_ctf.getValue(EMDL::CTF_VOLTAGE, _volt);            volt.push_back(_volt);
        if (MD_local_ctf.containsLabel(EMDL::CTF_FOM))
            MD_local_ctf.getValue(EMDL::CTF_FOM, _fom); fom.push_back(_fom);
        if (MD_local_ctf.containsLabel(EMDL::CTF_MAXRES))
            MD_local_ctf.getValue(EMDL::CTF_MAXRES, _maxres); maxres.push_back(_maxres);
        if (MD_local_ctf.containsLabel(EMDL::CTF_BFACTOR))
            MD_local_ctf.getValue(EMDL::CTF_BFACTOR, _bfac); bfac.push_back(_bfac);
        if (MD_local_ctf.containsLabel(EMDL::CTF_SCALEFACTOR))
            MD_local_ctf.getValue(EMDL::CTF_SCALEFACTOR, _sfac); sfac.push_back(_sfac);
        if (MD_local_ctf.containsLabel(EMDL::CTF_PHASESHIFT))
            MD_local_ctf.getValue(EMDL::CTF_PHASESHIFT, _phase); phase.push_back(_phase);
    }

    if (!MD_priors.containsLabel(EMDL::CTF_DEFOCUSU))
        MD_priors.addLabel(EMDL::CTF_DEFOCUSU);
    if (!MD_priors.containsLabel(EMDL::CTF_DEFOCUSV))
        MD_priors.addLabel(EMDL::CTF_DEFOCUSV);
    if (!MD_priors.containsLabel(EMDL::CTF_DEFOCUS_ANGLE))
        MD_priors.addLabel(EMDL::CTF_DEFOCUS_ANGLE);
    if (!MD_priors.containsLabel(EMDL::CTF_CS))
        MD_priors.addLabel(EMDL::CTF_CS);
    if (!MD_priors.containsLabel(EMDL::CTF_DETECTOR_PIXEL_SIZE))
        MD_priors.addLabel(EMDL::CTF_DETECTOR_PIXEL_SIZE);
    if (!MD_priors.containsLabel(EMDL::CTF_MAGNIFICATION))
        MD_priors.addLabel(EMDL::CTF_MAGNIFICATION);
    if (!MD_priors.containsLabel(EMDL::CTF_Q0))
        MD_priors.addLabel(EMDL::CTF_Q0);
    if (!MD_priors.containsLabel(EMDL::CTF_VOLTAGE))
        MD_priors.addLabel(EMDL::CTF_VOLTAGE);
    if (!MD_priors.containsLabel(EMDL::CTF_FOM))
        MD_priors.addLabel(EMDL::CTF_FOM);
    if (!MD_priors.containsLabel(EMDL::CTF_MAXRES))
        MD_priors.addLabel(EMDL::CTF_MAXRES);
    if (!MD_priors.containsLabel(EMDL::CTF_BFACTOR))
        MD_priors.addLabel(EMDL::CTF_BFACTOR);
    if (!MD_priors.containsLabel(EMDL::CTF_SCALEFACTOR))
        MD_priors.addLabel(EMDL::CTF_SCALEFACTOR);
    if (!MD_priors.containsLabel(EMDL::CTF_PHASESHIFT))
        MD_priors.addLabel(EMDL::CTF_PHASESHIFT);

    ii = -1;
    FOR_ALL_OBJECTS_IN_METADATA_TABLE(MD_priors) {
        ii++;
        MD_priors.getValue(EMDL::IMAGE_COORD_X, _x);
        MD_priors.getValue(EMDL::IMAGE_COORD_Y, _y);
        ...
        //MD_priors.setValue(EMDL::IMAGE_COORD_X, x[ii]);
        //MD_priors.setValue(EMDL::IMAGE_COORD_Y, y[ii]);
        MD_priors.setValue(EMDL::CTF_DEFOCUSU, dU[ii]);
        MD_priors.setValue(EMDL::CTF_DEFOCUSV, dV[ii]);
        MD_priors.setValue(EMDL::CTF_DEFOCUS_ANGLE, dAng[ii]);
        MD_priors.setValue(EMDL::CTF_CS, Cs[ii]);
        MD_priors.setValue(EMDL::CTF_DETECTOR_PIXEL_SIZE, pix[ii]);
        MD_priors.setValue(EMDL::CTF_MAGNIFICATION, mag[ii]);
        MD_priors.setValue(EMDL::CTF_Q0, Q0[ii]);
        MD_priors.setValue(EMDL::CTF_VOLTAGE, volt[ii]);
        if (MD_local_ctf.containsLabel(EMDL::CTF_FOM))
            MD_priors.setValue(EMDL::CTF_FOM, fom[ii]);
        if (MD_local_ctf.containsLabel(EMDL::CTF_MAXRES))
            MD_priors.setValue(EMDL::CTF_MAXRES, maxres[ii]);
        if (MD_local_ctf.containsLabel(EMDL::CTF_BFACTOR))
            MD_priors.setValue(EMDL::CTF_BFACTOR, bfac[ii]);
        if (MD_local_ctf.containsLabel(EMDL::CTF_SCALEFACTOR))
            MD_priors.setValue(EMDL::CTF_SCALEFACTOR, sfac[ii]);
        if (MD_local_ctf.containsLabel(EMDL::CTF_PHASESHIFT))
            MD_priors.setValue(EMDL::CTF_PHASESHIFT, phase[ii]);
    }
...
void combineParticlePriorsWithKaiLocalCTF_Multiple(
    std::string& suffix_priors, std::string& suffix_local_ctf, std::string& suffix_combined
) {
    FileName fns_priors;
...
void setNullTiltPriorsInDataStar(FileName& fn_in, FileName& fn_out) {
    MetaDataTable MD;
    ...
    MD.read(fn_in);
    if (!MD.containsLabel(EMDL::ORIENT_TILT_PRIOR))
        MD.addLabel(EMDL::ORIENT_TILT_PRIOR);
    FOR_ALL_OBJECTS_IN_METADATA_TABLE(MD) {
        MD.setValue(EMDL::ORIENT_TILT_PRIOR, 90.0);
    }
    MD.write(fn_out);
    return;
}
...
void removeBadTiltHelicalSegmentsFromDataStar(
    ...
    RFLOAT tilt_deg;
    if (max_dev_deg < 0.0 || max_dev_deg > 89.0)
    ...
    MD_out.clear();
    MD_in.read(fn_in);
    /// TODO: Use EMDL::ORIENT_TILT or EMDL::ORIENT_TILT_PRIOR ?
    if (!MD_in.containsLabel(EMDL::ORIENT_TILT))
    ...
    FOR_ALL_OBJECTS_IN_METADATA_TABLE(MD_in) {
        nr_segments_old++;
        MD_in.getValue(EMDL::ORIENT_TILT, tilt_deg);
...
void removeBadPsiHelicalSegmentsFromDataStar(
    ...
    MD_out.clear();
    MD_in.read(fn_in);
    if (!MD_in.containsLabel(EMDL::ORIENT_PSI) || !MD_in.containsLabel(EMDL::ORIENT_PSI_PRIOR))
        REPORT_ERROR("helix.cpp::removeBadTiltParticlesFromDataStar(): Input .star file contains no psi angles with their priors.");

    nr_segments_old = nr_segments_new = 0;
    FOR_ALL_OBJECTS_IN_METADATA_TABLE(MD_in) {
        nr_segments_old++;
        MD_in.getValue(EMDL::ORIENT_PSI, psi_deg);
        MD_in.getValue(EMDL::ORIENT_PSI_PRIOR, psi_prior_deg);
...
void convertHelicalSegmentCoordsToStarFile_Multiple(
        FileName& suffix_coords, FileName& suffix_out,
        int format_tag,
        RFLOAT pixel_size_A, RFLOAT Xdim, RFLOAT Ydim, RFLOAT boxsize,
        bool bimodal_angular_priors
) {
    ...
    for (int ii = 0; ii < fn_coords_list.size(); ii++) {
        ...
        if (format_tag == XIMDISP_COORDS_FORMAT) {
            convertXimdispHelicalSegmentCoordsToMetaDataTable(
                fn_coords_list[ii], MD_out, nr_segments, nr_tubes,
                pixel_size_A, Xdim, Ydim, boxsize, bimodal_angular_priors
            );
        } else if (format_tag == EMAN2_FORMAT) {
...
void convertHelicalSegmentCoordsToMetaDataTable(
    FileName& fn_in, MetaDataTable& MD_out,
    int& total_segments, bool is_3D_data,
    RFLOAT Xdim, RFLOAT Ydim, RFLOAT Zdim, RFLOAT box_size_pix,
    bool bimodal_angular_priors
) {
    MetaDataTable MD_in;
    if (fn_in.getExtension() != "star")
    ...
    if (
        !MD_in.containsLabel(EMDL::IMAGE_COORD_X) ||
        !MD_in.containsLabel(EMDL::IMAGE_COORD_Y) ||
        (is_3D_data && !MD_in.containsLabel(EMDL::IMAGE_COORD_Z)) ||
        !MD_in.containsLabel(EMDL::PARTICLE_HELICAL_TUBE_ID) ||
        !MD_in.containsLabel(EMDL::ORIENT_TILT_PRIOR) ||
        !MD_in.containsLabel(EMDL::ORIENT_PSI_PRIOR) ||
        !MD_in.containsLabel(EMDL::PARTICLE_HELICAL_TRACK_LENGTH_ANGSTROM)
    ...
    z = 1.0;
    FOR_ALL_OBJECTS_IN_METADATA_TABLE(MD_in) {
        MD_in.getValue(EMDL::IMAGE_COORD_X, x);
        MD_in.getValue(EMDL::IMAGE_COORD_Y, y);
        if (is_3D_data)
            MD_in.getValue(EMDL::IMAGE_COORD_Z, z);
        #ifdef EXCLUDE_SEGMENTS_ON_THE_EDGES
        // Avoid segments lying on the edges of the micrographs
        if (
...
void convertXimdispHelicalSegmentCoordsToMetaDataTable(
    FileName& fn_in, MetaDataTable& MD_out,
    int& total_segments, int& total_tubes,
    RFLOAT pixel_size_A, RFLOAT Xdim, RFLOAT Ydim, RFLOAT box_size_pix,
    bool bimodal_angular_priors
) {
    ...
    while (getline(fin, line, '\n')) {
        ...
        nr_segments++;
        MD_out.addObject();
        MD_out.setValue(EMDL::IMAGE_COORD_X, x);
        MD_out.setValue(EMDL::IMAGE_COORD_Y, y);
        MD_out.setValue(EMDL::PARTICLE_HELICAL_TUBE_ID, nr_tubes);
        MD_out.setValue(EMDL::ORIENT_TILT_PRIOR, 90.0);
        MD_out.setValue(EMDL::ORIENT_PSI_PRIOR, -psi_deg);
        MD_out.setValue(EMDL::PARTICLE_HELICAL_TRACK_LENGTH_ANGSTROM, pixel_size_A * len_pix);
        MD_out.setValue(EMDL::ORIENT_PSI_PRIOR_FLIP_RATIO, psi_prior_flip_ratio);
...
void convertXimdispHelicalTubeCoordsToMetaDataTable(
    FileName& fn_in, MetaDataTable& MD_out,
    int& total_segments, int& total_tubes, int nr_asu,
    RFLOAT rise_A, RFLOAT pixel_size_A, RFLOAT Xdim, RFLOAT Ydim, RFLOAT box_size_pix,
    bool bimodal_angular_priors, bool cut_into_segments
) {
    ...
    MD_out.addLabel(EMDL::ORIENT_TILT_PRIOR);
    MD_out.addLabel(EMDL::ORIENT_PSI_PRIOR);
    MD_out.addLabel(EMDL::PARTICLE_HELICAL_TRACK_LENGTH);
    MD_out.addLabel(EMDL::ORIENT_PSI_PRIOR_FLIP_RATIO);
    ...
    while (getline(fin, line, '\n')) {
        ...
        // Calculate start- and end-points for this helical tube
        x1 = (x[0] + x[1]) / 2.0;
        y1 = (y[0] + y[1]) / 2.0;
        x2 = (x[2] + x[3]) / 2.0;
        y2 = (y[2] + y[3]) / 2.0;
...
void convertEmanHelicalSegmentCoordsToMetaDataTable(
    ...
    if (
        box_size_pix < 2 || Xdim < box_size_pix || Ydim < box_size_pix
...
void convertEmanHelicalTubeCoordsToMetaDataTable(
    ...
    while (getline(fin, line, '\n')) {
        ...
        RFLOAT trans_offset = 0.0;
        x1 += (width1 / 2.0 - trans_offset) * cos(psi_rad);
        y1 += (width1 / 2.0 - trans_offset) * sin(psi_rad);
        x2 -= (width1 / 2.0 - trans_offset) * cos(psi_rad);
        ...
        int random_seed)
{
    ...
    if (!MD.containsLabel(EMDL::PARTICLE_RANDOM_SUBSET))
        MD.addLabel(EMDL::PARTICLE_RANDOM_SUBSET);
    if (MD.containsLabel(EMDL::PARTICLE_HELICAL_TUBE_ID))
    ...
    {
        mic_name.clear();
        MD.getValue(EMDL::MICROGRAPH_NAME, mic_name);
        if (divide_according_to_helical_tube_id)
        {
            MD.getValue(EMDL::PARTICLE_HELICAL_TUBE_ID, helical_tube_id);
    ...
    FOR_ALL_OBJECTS_IN_METADATA_TABLE(MD)
    {
        ...
        MD.setValue(EMDL::PARTICLE_RANDOM_SUBSET, map_mics[mic_name]);
    }
...
void makeHelicalReference2D(
    ...
    if (particle_diameter_pix < 2.0 || particle_diameter_pix > box_size)
    ...
    r2 = particle_diameter_pix * particle_diameter_pix / 4.0;
    t2 = tube_diameter_pix     * tube_diameter_pix     / 4.0;
...
void makeHelicalReference3DWithPolarity(
    ...
    RFLOAT cyl_diameter_A, RFLOAT topbottom_ratio,
    int sym_Cn, int nr_filaments_helix_with_seam
) {
    ...
    if (topbottom_ratio < 0.0 || topbottom_ratio > 1.0)
    ...
    if (cyl_radius_pix < 1.0 || cyl_radius_pix > particle_radius_pix)
    ...
    for (int id = 0; true; id++) {
        ...
        if (nr_filaments_helix_with_seam > 1) {
            if (id % nr_filaments_helix_with_seam == 0)
                append_additional_densities = !append_additional_densities;
            ...
            if (append_additional_densities) {
                ...
                z1 += particle_diameter_pix / 2.0;

                // Loop over x, y, z
                for (int dz = -particle_radius_max_pix / 2.0; dz <= particle_radius_max_pix / 2.0; dz++)
...
void divideStarFile(FileName& fn_in, int nr) {
    FileName fn_out;
...
void sortHelicalTubeID(MetaDataTable& MD) {
    ...
    if (
        !MD.containsLabel(EMDL::IMAGE_NAME) ||
        !MD.containsLabel(EMDL::ORIENT_TILT) ||
        !MD.containsLabel(EMDL::ORIENT_PSI) ||
        !MD.containsLabel(EMDL::ORIENT_TILT_PRIOR) ||
        !MD.containsLabel(EMDL::ORIENT_PSI_PRIOR) ||
        !MD.containsLabel(EMDL::PARTICLE_HELICAL_TUBE_ID) ||
        !MD.containsLabel(EMDL::PARTICLE_HELICAL_TRACK_LENGTH_ANGSTROM) ||
        !MD.containsLabel(EMDL::ORIENT_PSI_PRIOR_FLIP_RATIO)
    ) REPORT_ERROR("helix.cpp::sortHelicalTubeID: Labels of helical prior information are missing!");

    int_tube_id = 0;
    FOR_ALL_OBJECTS_IN_METADATA_TABLE(MD) {
        MD.getValue(EMDL::IMAGE_NAME, str_particle_fullname);
        MD.getValue(EMDL::PARTICLE_HELICAL_TUBE_ID, int_tube_id);
        ...
        MD.setValue(EMDL::IMAGE_NAME, str_comment);
    }
    ...
    FOR_ALL_OBJECTS_IN_METADATA_TABLE(MD) {
        MD.getValue(EMDL::IMAGE_NAME, str_comment);
    ...
    updatePriorsForHelicalReconstruction(
        MD, 1.0, dummy, dummy, 1,
        false, false,
        0.0, 0.0, 0.0, 1.0, false, 1
    );
...
void simulateHelicalSegments(
    bool is_3d_tomo,
    FileName& fn_vol_in, FileName& fn_star_out,
    RFLOAT white_noise,
    int new_box_size, int nr_subunits, int nr_asu, int nr_tubes,
    bool do_bimodal_searches,
    RFLOAT cyl_outer_diameter_A,
    RFLOAT angpix, RFLOAT rise_A, RFLOAT twist_deg,
    RFLOAT sigma_psi, RFLOAT sigma_tilt, RFLOAT sigma_offset,
    int random_seed
) {
    Image<RFLOAT> img;
    ...
    if (!fout) REPORT_ERROR((std::string) "helix.cpp::simulateHelicalSegments: Cannot write to .sh script!");

    // Enforce 0.0 lower bound
    sigma_tilt   = sigma_tilt   > 0.0 ? sigma_tilt   : 0.0;
    sigma_psi    = sigma_psi    > 0.0 ? sigma_psi    : 0.0;
    sigma_offset = sigma_offset > 0.0 ? sigma_offset : 0.0;
    ...
    MD.addLabel(EMDL::PARTICLE_HELICAL_TRACK_LENGTH_ANGSTROM);
    MD.addLabel(EMDL::ORIENT_PSI_PRIOR_FLIP_RATIO);
    MD.addLabel(EMDL::PARTICLE_HELICAL_TUBE_ID);
    MD.addLabel(EMDL::IMAGE_NAME);
    MD.addLabel(EMDL::MICROGRAPH_NAME);
    MD.addLabel(EMDL::CTF_DETECTOR_PIXEL_SIZE);
    MD.addLabel(EMDL::CTF_MAGNIFICATION);

    tube_id = 0;
    step_pix = nr_asu * rise_A / angpix;
    psi_flip_ratio = do_bimodal_searches ? 0.5 : 0.0;
    for (int id = 0; id < nr_segments; id++) {
        ...
        if (sigma_tilt < 0.0001) {
            ...
            new_tilt = tilt + rnd_gaus(0.0, sigma_tilt);
        }
        new_tilt = new_tilt < 0.0 ? -new_tilt : new_tilt; // Do NOT change the polarities
        new_tilt = new_tilt > 180.0 ? 360.0 - new_tilt : new_tilt;

        if (sigma_psi < 0.0001) {
            new_psi = psi;
        } else {
            new_psi = psi + rnd_gaus(0.0, sigma_psi);
        }
        ...
        MD.setValue(EMDL::ORIENT_TILT, new_tilt);
        MD.setValue(EMDL::ORIENT_PSI, new_psi);
        MD.setValue(EMDL::ORIENT_ORIGIN_X_ANGSTROM, xoff);
        MD.setValue(EMDL::ORIENT_ORIGIN_Y_ANGSTROM, yoff);
        if (is_3d_tomo)
            MD.setValue(EMDL::ORIENT_ORIGIN_Z_ANGSTROM, zoff);
        MD.setValue(EMDL::ORIENT_ROT_PRIOR, rot);
        MD.setValue(EMDL::ORIENT_TILT_PRIOR, new_tilt);
        MD.setValue(EMDL::ORIENT_PSI_PRIOR, new_psi);
        MD.setValue(EMDL::PARTICLE_HELICAL_TRACK_LENGTH_ANGSTROM, angpix * len_pix);
        ...
        if (is_3d_tomo) {
            MD.setValue(EMDL::MICROGRAPH_NAME, (std::string)("tomogram-01.mrc"));
        } else {
            MD.setValue(EMDL::MICROGRAPH_NAME, (std::string)("micrograph-01.mrc"));
        }
        MD.setValue(EMDL::CTF_DETECTOR_PIXEL_SIZE, 14.0);
        MD.setValue(EMDL::CTF_MAGNIFICATION, 140000.0 / angpix);
    ...
    MD.deactivateLabel(EMDL::ORIENT_ROT_PRIOR);
    FOR_ALL_OBJECTS_IN_METADATA_TABLE(MD) {
        MD.setValue(EMDL::ORIENT_ROT, 0.0);
        if (!is_3d_tomo) {
            MD.setValue(EMDL::ORIENT_TILT,       90.0);
            MD.setValue(EMDL::ORIENT_TILT_PRIOR, 90.0);
        }
        MD.setValue(EMDL::ORIENT_ORIGIN_X_ANGSTROM, 0.0);
        MD.setValue(EMDL::ORIENT_ORIGIN_Y_ANGSTROM, 0.0);
        if (is_3d_tomo)
            MD.setValue(EMDL::ORIENT_ORIGIN_Z_ANGSTROM, 0.0);

        MD.getValue(EMDL::IMAGE_NAME, fn_mic);
    ...
    FOR_ALL_OBJECTS_IN_METADATA_TABLE(MD) {
        MD.setValue(EMDL::ORIENT_ROT,  0.0);
        MD.setValue(EMDL::ORIENT_TILT, 0.0);
        MD.setValue(EMDL::ORIENT_PSI,  0.0);
    }
    MD.deactivateLabel(EMDL::ORIENT_TILT_PRIOR);
    MD.deactivateLabel(EMDL::ORIENT_PSI_PRIOR);
    MD.deactivateLabel(EMDL::PARTICLE_HELICAL_TRACK_LENGTH_ANGSTROM);
    MD.deactivateLabel(EMDL::ORIENT_PSI_PRIOR_FLIP_RATIO);
    MD.deactivateLabel(EMDL::PARTICLE_HELICAL_TUBE_ID);
    MD.write(fn_star_out_wopriors);

    command = is_3d_tomo ?
        "chmod u+x simulate-3d-subtomos.sh" :
        "chmod u+x simulate-2d-segments.sh";
...
void outputHelicalSymmetryStatus(
    ...
    RFLOAT rise_A_half1,    RFLOAT rise_A_half2,
    RFLOAT twist_deg_half1, RFLOAT twist_deg_half2,
    ...
    if (do_split_random_halves) {
        RFLOAT twist_avg_deg = (twist_deg_half1 + twist_deg_half2) / 2.0;
        RFLOAT rise_avg_A =    (rise_A_half1    + rise_A_half2)    / 2.0;
...
void excludeLowCTFCCMicrographs(
    ...
    RFLOAT cc_min, RFLOAT EPA_lowest_res,
    RFLOAT df_min, RFLOAT df_max
) {
    ...
    MD_in.read(fn_in);
    if (
        !MD_in.containsLabel(EMDL::CTF_DEFOCUSU) ||
        !MD_in.containsLabel(EMDL::CTF_DEFOCUSV) ||
        !MD_in.containsLabel(EMDL::CTF_DEFOCUS_ANGLE) ||
        !MD_in.containsLabel(EMDL::CTF_VOLTAGE) ||
        !MD_in.containsLabel(EMDL::CTF_CS) ||
        !MD_in.containsLabel(EMDL::CTF_Q0) ||
        !MD_in.containsLabel(EMDL::CTF_MAGNIFICATION) ||
        !MD_in.containsLabel(EMDL::CTF_DETECTOR_PIXEL_SIZE) ||
        !MD_in.containsLabel(EMDL::CTF_FOM)
    ) REPORT_ERROR("helix.cpp::excludeLowCTFCCMicrographs(): Input STAR file should contain CTF information.");

    contain_EPA_res = MD_in.containsLabel(EMDL::CTF_MAXRES);

    nr_mics_old = nr_mics_new = 0;
    MD_out.clear();
    FOR_ALL_OBJECTS_IN_METADATA_TABLE(MD_in) {
        nr_mics_old++;
        MD_in.getValue(EMDL::CTF_FOM, cc);
        MD_in.getValue(EMDL::CTF_MAXRES, EPA_res);
        MD_in.getValue(EMDL::CTF_DEFOCUSU, dU);
        MD_in.getValue(EMDL::CTF_DEFOCUSV, dV);
        if (
            cc > cc_min &&
            dU > df_min && dU < df_max &&
            dV > df_min && dV < df_max
...
void updatePriorsForOneHelicalTube(
    std::vector<HelicalSegmentPriorInfoEntry>& list,
    int sid, int eid,
    int& nr_wrong_polarity, bool &reverse_direction,
    RFLOAT sigma_segment_dist,
    std::vector<RFLOAT> helical_rise, std::vector<RFLOAT> helical_twist,
    bool is_3D_data, bool do_auto_refine,
    RFLOAT sigma2_rot, RFLOAT sigma2_tilt, RFLOAT sigma2_psi, RFLOAT sigma2_offset, RFLOAT sigma_cutoff
    // KThurber
) {
    ...
    sigma2_rot    = sigma2_rot    > 0.0 ? sigma2_rot    : 0.0;  // KThurber
    sigma2_tilt   = sigma2_tilt   > 0.0 ? sigma2_tilt   : 0.0;
    sigma2_psi    = sigma2_psi    > 0.0 ? sigma2_psi    : 0.0;
    sigma2_offset = sigma2_offset > 0.0 ? sigma2_offset : 0.0;
    ...
    if (psi_flip_ratio > 0.5) {
        for (int id = sid; id <= eid; id++) {
            ...
            list[id].psi_flip_ratio = 1.0 - psi_flip_ratio;
        }
    }

    // Calculate new distance-averaged angular priors
    // SHWS 27 Apr 2020: do two passes: one normal and one with opposite distances and find out which one is the best
    RFLOAT delta_prior_straight = 0.0, delta_prior_opposite = 0.0;
    for (int iflip = 0; iflip < 2; iflip++) {

        RFLOAT delta_prior = 0.0;
        for (int id = sid; id <= eid; id++) {
            ...
            // Calculate weights
            sum_w = this_w = do_avg ? gaussian1D(this_pos, sigma_segment_dist, center_pos) : 1.0;
            ...
            if (do_avg) {
                for (int idd = sid; idd <= eid; idd++) {
                    ...
                    transformCartesianAndHelicalCoords(
                        ...
                        is_3D_data ? this_rot  : 0.0,
                        is_3D_data ? this_tilt : 0.0,
                        this_psi, CART_TO_HELICAL_COORDS
                    );
                    if (is_3D_data) {
                        ZZ(this_trans_vec) = 0.0;
                    } else {
                ...
                if (diff > 180.0) diff = fabs(diff - 360.0);
                ...
                if (diff > 90.0) diff = fabs(diff - 180.0);
                delta_prior += diff;
        ...
        if (iflip == 1) {
            delta_prior_opposite = delta_prior / (eid - sid + 1);
        } else {
            delta_prior_straight = delta_prior / (eid - sid + 1);
        }
...
void updatePriorsForHelicalReconstruction(
    ...
    if (
        !MD.containsLabel(EMDL::ORIENT_TILT) ||
        !MD.containsLabel(EMDL::ORIENT_PSI) ||
        (is_3D_data && !MD.containsLabel(EMDL::ORIENT_ROT)) ||
        !MD.containsLabel(EMDL::ORIENT_ORIGIN_X_ANGSTROM) ||
        !MD.containsLabel(EMDL::ORIENT_ORIGIN_Y_ANGSTROM) ||
        (is_3D_data && !MD.containsLabel(EMDL::ORIENT_ORIGIN_Z_ANGSTROM)) ||
        !MD.containsLabel(EMDL::ORIENT_TILT_PRIOR) ||
        !MD.containsLabel(EMDL::ORIENT_PSI_PRIOR) ||
        !MD.containsLabel(EMDL::PARTICLE_HELICAL_TUBE_ID) ||
        !MD.containsLabel(EMDL::PARTICLE_HELICAL_TRACK_LENGTH_ANGSTROM) ||
        !MD.containsLabel(EMDL::ORIENT_PSI_PRIOR_FLIP_RATIO) ||
        (do_auto_refine && !MD.containsLabel(EMDL::PARTICLE_RANDOM_SUBSET))
    ...
    if (helical_nstart > 1) {
        ...
        for (int iclass = 0; iclass < helical_rise.size(); iclass++) {
            ...
            helical_twist[iclass] = aux > 360.0 ? aux + 360.0 : aux - 360.0;
            if (verb > 0) {
                std::cout << " + for rotational priors go back to " << helical_nstart
    ...
    FOR_ALL_OBJECTS_IN_METADATA_TABLE(MD) {
        ...
        MD.getValue(EMDL::MICROGRAPH_NAME, str_mic);
        MD.getValue(EMDL::PARTICLE_HELICAL_TUBE_ID, tube_id);
        segment.helical_tube_name = str_mic + integerToString(tube_id);
        MD.getValue(EMDL::PARTICLE_HELICAL_TRACK_LENGTH_ANGSTROM, segment.track_pos_A);
        if (MD.containsLabel(EMDL::ORIENT_ROT)) {
            MD.getValue(EMDL::ORIENT_ROT, segment.rot_deg);  		// KThurber
        } else {
            segment.rot_deg = 0.0;
        }
        if (MD.containsLabel(EMDL::ORIENT_ROT_PRIOR)) {
            MD.getValue(EMDL::ORIENT_ROT_PRIOR, segment.rot_prior_deg);  	// KThurber
        } else {
            segment.rot_prior_deg = segment.rot_deg;  // SHWS, modified from KThurber!
            // segment.rot_prior_deg = 0.0;
        }
        MD.getValue(EMDL::ORIENT_TILT, segment.tilt_deg);
        MD.getValue(EMDL::ORIENT_TILT_PRIOR, segment.tilt_prior_deg);
        MD.getValue(EMDL::ORIENT_PSI, segment.psi_deg);
        MD.getValue(EMDL::ORIENT_PSI_PRIOR, segment.psi_prior_deg);
        MD.getValue(EMDL::ORIENT_PSI_PRIOR_FLIP_RATIO, segment.psi_flip_ratio);
        // KThurber2
        if (MD.containsLabel(EMDL::ORIENT_PSI_PRIOR_FLIP)) {
            MD.getValue(EMDL::ORIENT_PSI_PRIOR_FLIP, segment.psi_prior_flip);
        } else {
            segment.psi_prior_flip = false;
        }
        if (MD.containsLabel(EMDL::PARTICLE_CLASS)) {
            MD.getValue(EMDL::PARTICLE_CLASS, segment.classID);
        } else {
            segment.classID = 1;
        }
        if (do_auto_refine)
            MD.getValue(EMDL::PARTICLE_RANDOM_SUBSET, segment.subset); // Do I really need this?

        MD.getValue(EMDL::ORIENT_ORIGIN_X_ANGSTROM, segment.dx_A);
        MD.getValue(EMDL::ORIENT_ORIGIN_Y_ANGSTROM, segment.dy_A);
        if (is_3D_data)
            MD.getValue(EMDL::ORIENT_ORIGIN_Z_ANGSTROM, segment.dz_A);
    ...
    for (int sid = 0; sid < list.size();) {
        ...
        updatePriorsForOneHelicalTube(
            list, sid, eid, nr_opposite_polarity, reverse_direction, 
            sigma_segment_dist, helical_rise, helical_twist,
            is_3D_data, do_auto_refine, 
            sigma2_rot, sigma2_tilt, sigma2_psi, sigma2_offset
        );
        total_opposite_polarity += nr_opposite_polarity;
        if (reverse_direction) {
            total_opposite_rot += 1;
        } else {
            total_same_rot += 1;
        }
        ...
        for (int id = sid; id <= eid; id++) {
            if (reverse_direction)
                MD.setValue(EMDL::PARTICLE_HELICAL_TRACK_LENGTH_ANGSTROM, -1.0 * list[id].track_pos_A, list[id].MDobjectID);
            if (!keep_tilt_prior_fixed)
                MD.setValue(EMDL::ORIENT_TILT_PRIOR, list[id].tilt_prior_deg, list[id].MDobjectID);
            MD.setValue(EMDL::ORIENT_PSI_PRIOR,            list[id].psi_prior_deg,  list[id].MDobjectID);
            MD.setValue(EMDL::ORIENT_PSI_PRIOR_FLIP_RATIO, list[id].psi_flip_ratio, list[id].MDobjectID);
            MD.setValue(EMDL::ORIENT_ROT_PRIOR,            list[id].rot_prior_deg,  list[id].MDobjectID); // KThurber
            MD.setValue(EMDL::ORIENT_ORIGIN_X_ANGSTROM,    list[id].dx_prior_A,     list[id].MDobjectID);
            MD.setValue(EMDL::ORIENT_ORIGIN_Y_ANGSTROM,    list[id].dy_prior_A,     list[id].MDobjectID);
            if (is_3D_data)
                MD.setValue(EMDL::ORIENT_ORIGIN_Z_ANGSTROM, list[id].dz_prior_A, list[id].MDobjectID);
        }
...
void updateAngularPriorsForHelicalReconstructionFromLastIter(
    ...
    bool have_psi        = MD.containsLabel(EMDL::ORIENT_PSI);
    bool have_tilt_prior = MD.containsLabel(EMDL::ORIENT_TILT_PRIOR);
    bool have_psi_prior  = MD.containsLabel(EMDL::ORIENT_PSI_PRIOR);
    bool have_rot        = MD.containsLabel(EMDL::ORIENT_ROT);		// KThurber
    bool have_rot_prior  = MD.containsLabel(EMDL::ORIENT_ROT_PRIOR);	// KThurber

    if (!have_tilt_prior && !have_psi_prior && !have_rot_prior)	// KThurber
        return;

    // The conditionals within this loop will get evaluated on every iteration, 
    // but the result will remain the same.
    // Hopefully the compiler will notice this 
    // and be able to optimise away the overhead by hoisting this loop-invariant code.
    // https://en.wikipedia.org/wiki/Loop-invariant_code_motion
    FOR_ALL_OBJECTS_IN_METADATA_TABLE(MD) {
        RFLOAT val;
        if (have_tilt && have_tilt_prior && !keep_tilt_prior_fixed) {
            MD.getValue(EMDL::ORIENT_TILT, val);
            MD.setValue(EMDL::ORIENT_TILT_PRIOR, val);
        }
        if (have_psi && have_psi_prior) {
            MD.getValue(EMDL::ORIENT_PSI, val);
            MD.setValue(EMDL::ORIENT_PSI_PRIOR, val);
        }
        // KThurber add rot section
        if (have_rot && have_rot_prior) {
            MD.getValue(EMDL::ORIENT_ROT, val);
            MD.setValue(EMDL::ORIENT_ROT_PRIOR, val);
        }
    }
}

void setPsiFlipRatioInStarFile(MetaDataTable& MD, RFLOAT ratio) {
    if (!MD.containsLabel(EMDL::ORIENT_PSI_PRIOR_FLIP_RATIO))
        REPORT_ERROR("helix.cpp::setPsiFlipRatioInStarFile: Psi flip ratio is not found in this STAR file!");
    FOR_ALL_OBJECTS_IN_METADATA_TABLE(MD) {
        MD.setValue(EMDL::ORIENT_PSI_PRIOR_FLIP_RATIO, ratio);
    }
}

void plotLatticePoints(MetaDataTable& MD, int x1, int y1, int x2, int y2) {
    MD.clear();
    MD.addLabel(EMDL::IMAGE_COORD_X);
    MD.addLabel(EMDL::IMAGE_COORD_Y);
    for (int i = -10; i <= 10; i++)
    for (int j = -10; j <= 10; j++) {
        MD.addObject();
        MD.setValue(EMDL::IMAGE_COORD_X, RFLOAT(i * x1 + j * x2));
        MD.setValue(EMDL::IMAGE_COORD_Y, RFLOAT(i * y1 + j * y2));
    }
}

void grabParticleCoordinates(FileName& fn_in, FileName& fn_out) {
    MetaDataTable MD_in, MD_out;
    RFLOAT x, y, z;
    ...
    FOR_ALL_OBJECTS_IN_METADATA_TABLE(MD_in) {
        ...
        MD_out.setValue(EMDL::IMAGE_COORD_Y, y);
        if (contain_z_coord)
            MD_out.setValue(EMDL::IMAGE_COORD_Z, z);
    }
    MD_out.write(fn_out);
...
void grabParticleCoordinates_Multiple(std::string& suffix_fin, std::string& suffix_fout) {
    FileName fns_in;
    std::vector<FileName> fn_in_list;
...
void transformCartesianToHelicalCoordsForStarFiles(MetaDataTable& MD_in, MetaDataTable& MD_out) {
    ...
    FOR_ALL_OBJECTS_IN_METADATA_TABLE(MD_in) {
        MD_in.getValue(EMDL::ORIENT_ROT, rot);
        MD_in.getValue(EMDL::ORIENT_TILT, tilt);
        MD_in.getValue(EMDL::ORIENT_PSI, psi);
        MD_in.getValue(EMDL::ORIENT_ORIGIN_X_ANGSTROM, xoff);
        MD_in.getValue(EMDL::ORIENT_ORIGIN_Y_ANGSTROM, yoff);
        if (is_3d_trans)
            MD_in.getValue(EMDL::ORIENT_ORIGIN_Z_ANGSTROM, zoff);
...
void normaliseHelicalSegments(
    FileName& fn_in, FileName& fn_out_root,
    ...
    FOR_ALL_OBJECTS_IN_METADATA_TABLE(MD) {
        ...
        MD.getValue(have_tilt_prior ? EMDL::ORIENT_TILT_PRIOR : EMDL::ORIENT_TILT, tilt_deg);
        MD.getValue(have_psi_prior  ? EMDL::ORIENT_PSI_PRIOR  : EMDL::ORIENT_PSI,  psi_deg);
        if (read_angpix_from_star) {
            MD.getValue(EMDL::CTF_DETECTOR_PIXEL_SIZE, det_pixel_size);
            MD.getValue(EMDL::CTF_MAGNIFICATION, mag);
        ...
        if (helical_outer_diameter_A / pixel_size_A < 10.0)
...
void HermiteInterpolateOne3DHelicalFilament(
    MetaDataTable& MD_in, MetaDataTable& MD_out,
    int& total_segments, int nr_asu,
    RFLOAT rise_A, RFLOAT pixel_size_A, RFLOAT box_size_pix,
    int helical_tube_id,
    RFLOAT Xdim, RFLOAT Ydim, RFLOAT Zdim,
    bool bimodal_angular_priors
) {
    ...
    if (
        !MD_in.containsLabel(EMDL::IMAGE_COORD_X) || 
        !MD_in.containsLabel(EMDL::IMAGE_COORD_Y) || 
        !MD_in.containsLabel(EMDL::IMAGE_COORD_Z)
    ...
    MD_out.addLabel(EMDL::IMAGE_COORD_X);
    MD_out.addLabel(EMDL::IMAGE_COORD_Y);
    MD_out.addLabel(EMDL::IMAGE_COORD_Z);
    MD_out.addLabel(EMDL::PARTICLE_HELICAL_TUBE_ID);
    MD_out.addLabel(EMDL::ORIENT_TILT_PRIOR);
    MD_out.addLabel(EMDL::ORIENT_PSI_PRIOR);
    MD_out.addLabel(EMDL::PARTICLE_HELICAL_TRACK_LENGTH_ANGSTROM);
    ...
    // half_box_size_pix = box_size_pix / 2.0;
    psi_prior_flip_ratio = bimodal_angular_priors ? BIMODAL_PSI_PRIOR_FLIP_RATIO
        : UNIMODAL_PSI_PRIOR_FLIP_RATIO;

    // Load all manually picked coordinates
    xlist.clear(); ylist.clear(); zlist.clear();
    FOR_ALL_OBJECTS_IN_METADATA_TABLE(MD_in) {
        MD_in.getValue(EMDL::IMAGE_COORD_X, x0);
        MD_in.getValue(EMDL::IMAGE_COORD_Y, y0);
        MD_in.getValue(EMDL::IMAGE_COORD_Z, z0);
        xlist.push_back(x0);
        ylist.push_back(y0);
        zlist.push_back(z0);
    }
    ...
    for (int id = 0; id < (xlist.size() - 1); id++) {
        ...
        step_pix = interbox_pix < 57.735 ? 0.57735 : interbox_pix / 100.0; // 1%
        //step_pix = (interbox_pix < 5.7735) ? (0.57735) : (interbox_pix / 10.); // 10%

        // Collect points 0, 1, 2, 3 for interpolations
        x0 = x1 = x2 = x3 = y0 = y1 = y2 = y3 = z0 = z1 = z2 = z3 = 0.0;
        // Point 0
        if (id == 0) {
            x0 = 2.0 * xlist[id] - xlist[id + 1];
            y0 = 2.0 * ylist[id] - ylist[id + 1];
            z0 = 2.0 * zlist[id] - zlist[id + 1];
        } else {
            x0 = xlist[id - 1]; 
            y0 = ylist[id - 1]; 
            z0 = zlist[id - 1];
        ...
        // Point 3
        if (id == (xlist.size() - 2)) {
            x3 = 2.0 * xlist[id + 1] - xlist[id];
            y3 = 2.0 * ylist[id + 1] - ylist[id];
            z3 = 2.0 * zlist[id + 1] - zlist[id];
        } else {
            x3 = xlist[id + 2]; 
            y3 = ylist[id + 2]; 
            z3 = zlist[id + 2];
        ...
        for (int ip = 0; ip < nr_partitions; ip++) {
            ...
            if (present_len_pix > 0.0) {
                ...
                if (
                    xb < half_box_size_pix || xb > Xdim - half_box_size_pix || 
                ...
                nr_segments++;
                MD_out.addObject();
                MD_out.setValue(EMDL::IMAGE_COORD_X, xb);
                MD_out.setValue(EMDL::IMAGE_COORD_Y, yb);
                MD_out.setValue(EMDL::IMAGE_COORD_Z, zb);
                MD_out.setValue(EMDL::PARTICLE_HELICAL_TUBE_ID, helical_tube_id);
                ...
                MD_out.setValue(EMDL::PARTICLE_HELICAL_TRACK_LENGTH_ANGSTROM, accu_len_pix * pixel_size_A);
                MD_out.setValue(EMDL::ORIENT_PSI_PRIOR_FLIP_RATIO,            psi_prior_flip_ratio);
             }
        }
    }
...
void Interpolate3DCurves(
    FileName& fn_in_root, FileName& fn_out_root,
    int nr_asu,
    RFLOAT rise_A, RFLOAT pixel_size_A, RFLOAT box_size_pix,
    int binning_factor, bool bimodal_angular_priors
) {
    Image<RFLOAT> img;
    std::vector<FileName> fn_in_list;
    ...
    int xdim_img = std::get<0>(dimensions);
    int ydim_img = std::get<1>(dimensions);
    int zdim_img = std::get<2>(dimensions);
    long int ndim_img = std::get<3>(dimensions);
    if (zdim_img <= 1)
    ...
    for (int fid = 0; fid < fn_in_list.size(); fid++) {
        ...
        // Method 2
        if (
            fn_ext != "mod"    && fn_ext != "star" && 
            fn_ext != "coords" && fn_ext != "txt"
        ) {
            fn_in_list.erase(fn_in_list.begin() + fid);
            fid--;
            continue;
    ...
    for (int fid = 0; fid < fn_in_list.size(); fid++) {
        ...
        if (fn_in.getExtension() == "mod") {
            ...
            */
            ...
            contain_3d_points = false;
            // Read 4-byte blocks
            while (fin.read(reinterpret_cast<char*>(buf_word), sizeof(buf_word))) {
                // Find contour section
                if (buf_word[0] == 'C' && buf_word[1] == 'O' && buf_word[2] == 'N' && buf_word[3] == 'T') {
            ...
            FOR_ALL_OBJECTS_IN_METADATA_TABLE(MD) {
                MD.getValue(EMDL::IMAGE_COORD_X, xx);
                MD.getValue(EMDL::IMAGE_COORD_Y, yy);
                MD.getValue(EMDL::IMAGE_COORD_Z, zz);
                xlist.push_back(xx);
                ylist.push_back(yy);
                zlist.push_back(zz);
            }
        ...
        MD_in.clear();
        MD_in.addLabel(EMDL::IMAGE_COORD_X);
        ...
        for (int id = 0; id < xlist.size(); id++) {
            ...
            if (id == 0) {
                MD_in.addObject();
                MD_in.setValue(EMDL::IMAGE_COORD_X, xlist[0]);
                MD_in.setValue(EMDL::IMAGE_COORD_Y, ylist[0]);
                MD_in.setValue(EMDL::IMAGE_COORD_Z, zlist[0]);
                continue;
            }
            // Middle points of each short line segment
            MD_in.addObject();
            MD_in.setValue(EMDL::IMAGE_COORD_X, (xlist[id] + xlist[id - 1]) / 2.0);
            MD_in.setValue(EMDL::IMAGE_COORD_Y, (ylist[id] + ylist[id - 1]) / 2.0);
            MD_in.setValue(EMDL::IMAGE_COORD_Z, (zlist[id] + zlist[id - 1]) / 2.0);
            // End point
            if (id == xlist.size() - 1) {
                MD_in.addObject();
                MD_in.setValue(EMDL::IMAGE_COORD_X, xlist[id]);
...
void estimateTiltPsiPriors(
    ...
    RFLOAT tilt_rad = 0.0, psi_rad = 0.0, vec_len = 0.0;
    int dim = dr.size();
    ...
    if (dim == 3) {
        ...
        psi_rad = atan2(YY(dr), -1.0 * XX(dr)); // 'atan2' returns an angle within [-pi, +pi] radians for rot
    } else {
...
void select3DsubtomoFrom2Dproj(MetaDataTable& MD_2d, MetaDataTable& MD_3d, MetaDataTable& MD_out) {
    ...
    RFLOAT Dpix = -1.0, Mag = -1.0, _Dpix = -1.0, _Mag = -1.0;
    ...
    if (
        ...
        !MD_2d.containsLabel(EMDL::IMAGE_COORD_Y)   ||
        !MD_2d.containsLabel(EMDL::IMAGE_COORD_Z)   ||
        !MD_2d.containsLabel(EMDL::MICROGRAPH_NAME) ||
        !MD_2d.containsLabel(EMDL::IMAGE_NAME)
    ...
    if (
        ...
        !MD_3d.containsLabel(EMDL::IMAGE_COORD_Y)   ||
        !MD_3d.containsLabel(EMDL::IMAGE_COORD_Z)   ||
        !MD_3d.containsLabel(EMDL::MICROGRAPH_NAME) ||
        !MD_3d.containsLabel(EMDL::IMAGE_NAME)
    ...
    if (auto_pixel_size) {
        if (!MD_3d.containsLabel(EMDL::CTF_DETECTOR_PIXEL_SIZE) || !MD_3d.containsLabel(EMDL::CTF_MAGNIFICATION))
        ...
        FOR_ALL_OBJECTS_IN_METADATA_TABLE(MD_3d) {
            MD_3d.getValue(EMDL::CTF_DETECTOR_PIXEL_SIZE, _Dpix);
            MD_3d.getValue(EMDL::CTF_MAGNIFICATION, _Mag);
        ...
        FOR_ALL_OBJECTS_IN_METADATA_TABLE(MD_2d) {
            MD_2d.getValue(EMDL::CTF_DETECTOR_PIXEL_SIZE, _Dpix);
            MD_2d.getValue(EMDL::CTF_MAGNIFICATION, _Mag);
            if (_Dpix <= 0.0 || _Mag <= 0.0)
    ...
    mic_list.clear();
    FOR_ALL_OBJECTS_IN_METADATA_TABLE(MD_2d) {
        //MD_2d.getValue(EMDL::IMAGE_COORD_X, xx);
        //MD_2d.getValue(EMDL::IMAGE_COORD_Y, yy);
        //MD_2d.getValue(EMDL::IMAGE_COORD_Z, zz);
        MD_2d.getValue(EMDL::MICROGRAPH_NAME, mic_str);
        MD_2d.getValue(EMDL::IMAGE_NAME, img_str);
    ...
    MD_out.clear();
    FOR_ALL_OBJECTS_IN_METADATA_TABLE(MD_3d) {
        // MD_3d.getValue(EMDL::IMAGE_COORD_X, xx);
        // MD_3d.getValue(EMDL::IMAGE_COORD_Y, yy);
        // MD_3d.getValue(EMDL::IMAGE_COORD_Z, zz);
        MD_3d.getValue(EMDL::MICROGRAPH_NAME, mic_str);
        MD_3d.getValue(EMDL::IMAGE_NAME, img_str);
        ...
        if (std::binary_search(mic_list.begin(), mic_list.end(), img_str)) {
            ...
            if (auto_pixel_size) {
                MD_out.setValue(EMDL::CTF_DETECTOR_PIXEL_SIZE, Dpix);
                MD_out.setValue(EMDL::CTF_MAGNIFICATION, Mag);
            }
        }
    }
    // std::cout << std::endl;
...
void averageAsymmetricUnits2D(
    ObservationModel& obsModel, MetaDataTable &MDimgs,
    FileName fn_o_root, int nr_asu, RFLOAT rise
) {
    ...
    FOR_ALL_OBJECTS_IN_METADATA_TABLE(MDimgs) {
        ...
        MDimgs.getValue(EMDL::IMAGE_NAME, fn_img);
        MDimgs.getValue(EMDL::ORIENT_PSI, psi);
        MDimgs.getValue(EMDL::IMAGE_OPTICS_GROUP, optics_group);
        optics_group--;
