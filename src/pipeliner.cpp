...
void PipeLine::remakeNodeDirectory() {
    ...
    for (long int i = 0; i < nodeList.size(); i++) {
        ...
        bool touch_if_not_exist = myproc >= 0 && processList[myproc].status == PROC_SCHEDULED;
        touchTemporaryNodeFile(nodeList[i], touch_if_not_exist);
    }
...
bool PipeLine::checkProcessCompletion() {
    ...
    if (
        finished_success_processes.size() > 0 ||
...
bool PipeLine::runJob(
    ...
    */
    ...
    if (!only_schedule && is_main_continue) {
        if (
            processList[current_job].type == PROC_2DCLASS ||
            processList[current_job].type == PROC_3DCLASS ||
            processList[current_job].type == PROC_3DAUTO ||
            processList[current_job].type == PROC_MULTIBODY ||
            processList[current_job].type == PROC_INIMODEL
    ...
    int mynewstatus = only_schedule ? PROC_SCHEDULED : PROC_RUNNING;
...
int PipeLine::addScheduledJob(int job_type, std::string fn_options) {
    ...
    if (
        job_type == PROC_MOTIONCORR || job_type == PROC_CTFFIND || 
        job_type == PROC_AUTOPICK   || job_type == PROC_EXTRACT
    )
                ...
                if (
                    ...
                    if (processList[current_job].status == PROC_FINISHED_SUCCESS) {
                        ...
                        if (repeat + 1 != nr_repeat) {
                            ...
                            if (
                                mytype == PROC_MOTIONCORR || mytype == PROC_CTFFIND || 
                                mytype == PROC_AUTOPICK   || mytype == PROC_EXTRACT || 
...
bool PipeLine::markAsFinishedJob(
    ...
    if (
        processList[this_job].type == PROC_2DCLASS ||
...
bool PipeLine::cleanupJob(int this_job, bool do_harsh, std::string &error_message) {
    ...
    if (
        processList[this_job].type == PROC_IMPORT ||
...
void PipeLine::read(bool do_lock, std::string lock_message) {
    ...
    if (MDgen.readStar(in, "pipeline_general")) {
        int jobcounter = MDgen.getValue<int>(EMDL::PIPELINE_JOB_COUNTER);
    ...
    FOR_ALL_OBJECTS_IN_METADATA_TABLE(MDproc) {
        ...
        int type, status;
        try {
            name   = MDproc.getValue<std::string>(EMDL::PIPELINE_PROCESS_NAME);
            alias  = MDproc.getValue<std::string>(EMDL::PIPELINE_PROCESS_ALIAS);
            type   = MDproc.getValue<int>        (EMDL::PIPELINE_PROCESS_TYPE);
            status = MDproc.getValue<int>        (EMDL::PIPELINE_PROCESS_STATUS);
        } catch (const char *errmsg) {
    ...
    FOR_ALL_OBJECTS_IN_METADATA_TABLE(MDedge1) {
        ...
        try {
            ...
            fromnodename = MDedge1.getValue<std::string>(EMDL::PIPELINE_EDGE_FROM);
        } catch (const char *errmsg) {
    ...
    FOR_ALL_OBJECTS_IN_METADATA_TABLE(MDedge2) {
        ...
        try {
            ...
            procname   = MDedge2.getValue<std::string>(EMDL::PIPELINE_EDGE_PROCESS);
        } catch (const char *errmsg) {
...
void PipeLine::write(
    ...
    for (long int i = 0; i < processList.size(); i++) {
        for (long int j = 0; j < (processList[i].inputNodeList).size(); j++) {
            ...
            if (fn_del == "" || (!deleteProcess[i] && !deleteNode[inputNode])) {
                MDedge1.addObject();
                MDedge1.setValue(EMDL::PIPELINE_EDGE_FROM, nodeList[inputNode].name);
                MDedge1.setValue(EMDL::PIPELINE_EDGE_PROCESS, processList[i].name);
            } else {
                MDedge1_del.addObject();
                MDedge1_del.setValue(EMDL::PIPELINE_EDGE_FROM, nodeList[inputNode].name);
    ...
    for (long int j = 0; j < processList[i].outputNodeList.size(); j++) {
        ...
        if (fn_del == "" || (!deleteProcess[i] && !deleteNode[outputNode])) {
            ...
            MDedge2.setValue(EMDL::PIPELINE_EDGE_TO, nodeList[outputNode].name);
        } else {
            MDedge2_del.addObject();
            MDedge2_del.setValue(EMDL::PIPELINE_EDGE_PROCESS,  processList[i].name);
            MDedge2_del.setValue(EMDL::PIPELINE_EDGE_TO, nodeList[outputNode].name);
        }
    }
    MDedge2.write(fh);
    if (fn_del != "")
