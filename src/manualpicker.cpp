...
int manualpickerGuiWindow::fill() {
    ...
    global_has_ctf = MDin.containsLabel(EMDL::CTF_IMAGE);
    ...
    for (long int _ : MDin) {
        ...
        int ystep2 = ystep - (fn_mic.length() > MWCOL1 / 12 ? 5 : 10);
...
void ManualPicker::initialise() {
    ...
    if (global_micscale < 1.0) {
        RFLOAT new_nyquist = global_angpix * 2.0 / global_micscale;
