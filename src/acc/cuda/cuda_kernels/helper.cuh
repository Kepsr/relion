...
__global__ void cuda_kernel_calcPowerSpectrum(
    T *dFaux, int padoridim, T *ddata, int data_sz, RFLOAT *dpower_spectrum, RFLOAT *dcounter,
    ...
    int dx = data_sz / 2 + 1;
    int dxy = blockDim.z == 1 ? 0 : data_sz * dx;
    ...
    if (idx < XSIZE && idy < padoridim && idz < padoridim) {
        ...
        if (r2 <= max_r2) {
            ...
            atomicAdd(dpower_spectrum + ires, norm);    // direct::elem(power_spectrum, ires) += norm(data.elem(ip, jp, kp)) / 2.0;
            ...
            val.x = val.y = 0.;
            if (r2 <= min_r2)
                ddata[(kp + data_sz / 2) * dxy + (ip + data_sz / 2) * dx + jp] = val;  // data.elem(ip, jp, kp) = 0;
        }
    }
}

template __global__ void cuda_kernel_calcPowerSpectrum(
    double2*, int, double2*, int , RFLOAT*, RFLOAT*, int, int, RFLOAT, RFLOAT, RFLOAT,
    RFLOAT*, int, int, int, bool
);

template __global__ void cuda_kernel_calcPowerSpectrum(
    float2*, int, float2*, int , RFLOAT*, RFLOAT*, int, int, RFLOAT, RFLOAT, RFLOAT,
    RFLOAT*, int, int, int, bool
);
