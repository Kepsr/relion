...
__global__ void cuda_kernel_diff2_coarse(
    ...
    const int max_block_pass_euler = ceilfracf(eulers_per_block * 9, block_sz) * block_sz;

    for (int i = threadIdx.x; i < max_block_pass_euler; i += block_sz)
    ...
    XFLOAT tx = trans_x[threadIdx.x%translation_num];
    XFLOAT ty = trans_y[threadIdx.x%translation_num];
    XFLOAT tz = trans_z[threadIdx.x%translation_num];
    ...
    for (int init_pixel = 0; init_pixel < max_block_pass_pixel; init_pixel += block_sz/prefetch_fraction)
    {
        ...
        if(init_pixel + threadIdx.x/prefetch_fraction < image_size)
        {
            int x,y,z,xy;
            if(DATA3D)
            {
                z =  floorfracf(init_pixel + threadIdx.x/prefetch_fraction, projector.imgX*projector.imgY);
                xy = (init_pixel + threadIdx.x/prefetch_fraction) % (projector.imgX*projector.imgY);
                x =             xy  % projector.imgX;
            ...
            {
                x =           ( init_pixel + threadIdx.x/prefetch_fraction) % projector.imgX;
                y = floorfracf( init_pixel + threadIdx.x/prefetch_fraction  , projector.imgX);
            }
            if (y > projector.maxR)
                y -= projector.imgY;

//			#pragma unroll
            for (int i = threadIdx.x%prefetch_fraction; i < eulers_per_block; i += prefetch_fraction)
            {
                if(DATA3D) // if DATA3D, then REF3D as well.
                    projector.project3Dmodel(
                        ...
                        s_eulers[i*9+7],
                        s_eulers[i*9+8],
                        s_ref_real[eulers_per_block * (threadIdx.x/prefetch_fraction) + i],
                        s_ref_imag[eulers_per_block * (threadIdx.x/prefetch_fraction) + i]);
                else if(REF3D)
                    projector.project3Dmodel(
                        x,y,
                        s_eulers[i*9  ],
        ...
        //Prefetch block-wise
        if (init_pixel % block_sz == 0 && init_pixel + threadIdx.x < image_size)
        {
            s_real[threadIdx.x] = g_real[init_pixel + threadIdx.x];
            s_imag[threadIdx.x] = g_imag[init_pixel + threadIdx.x];
            s_corr[threadIdx.x] = g_corr[init_pixel + threadIdx.x] / 2;
        }

        __syncthreads();

        if (threadIdx.x/translation_num < block_sz/translation_num) // NOTE int division A/B==C/B !=> A==C
        for (int i = threadIdx.x / translation_num;
                ...
                i += block_sz/translation_num)
        {
            ...
            for (int j = 0; j < eulers_per_block; j++) {
                const XFLOAT diff_real = s_ref_real[eulers_per_block * i + j] - real;
                const XFLOAT diff_imag = s_ref_imag[eulers_per_block * i + j] - imag;
    ...
    for (int i = 0; i < eulers_per_block; i++) {
        const int j = blockIdx.x * eulers_per_block + i;
        cuda_atomic_add(g_diff2s + (j * translation_num + threadIdx.x % translation_num), diff2s[i]);
    }
}
