...
bool AccProjector::setMdlDim(
    ...
    if (
        xdim == mdlX && ydim == mdlY && zdim == mdlZ &&
        inity == mdlInitY && initz == mdlInitZ && 
        maxr == mdlMaxR &&
    ...
    mdlY = ydim;
    mdlZ = zdim;
    mdlXYZ = zdim == 0 ? (size_t) xdim * (size_t) ydim : (size_t) xdim * (size_t) ydim * (size_t) zdim;
    mdlInitY = inity;
    mdlInitZ = initz;
    mdlMaxR  = maxr;
...
void AccProjector::initMdl(Complex *data) {
    ...
    XFLOAT *tmpReal;
    if (posix_memalign((void **) &tmpReal, MEM_ALIGN, mdlXYZ * sizeof(XFLOAT))) 
    CRITICAL(RAMERR);

    XFLOAT *tmpImag;
    if (posix_memalign((void **) &tmpImag, MEM_ALIGN, mdlXYZ * sizeof(XFLOAT))) 
    CRITICAL(RAMERR);
