...
reconFn0(""), reconFn1(""), maskFn(""), fscFn(""),
paddingFactor(2.0), hasMask(false) {}
...
void ReferenceMap::load(int verb, bool debug) {
    ...
    if (
        maps[1].data.xdim != maps[1].data.ydim || 
    ...
    if (!fscFn.empty()) {
        ...
        if (!fscMdt.containsLabel(EMDL::SPECTRAL_IDX)) {
            REPORT_ERROR(fscFn + " does not contain a value for " + EMDL::label2Str(EMDL::SPECTRAL_IDX));
        }
...
Image<RFLOAT> ReferenceMap::getHollowWeight(double kmin_ang, int s_out, double angpix_out) {
...
Volume<t2Vector<Complex>> ReferenceMap::predictComplexGradient(
    ...
    pred = obs.predictComplexGradient(
        projectors[pi], mdt, p, angpix, 
        applyCtf, applyTilt, applyShift, applyMtf, applyCtfPadding
    );
