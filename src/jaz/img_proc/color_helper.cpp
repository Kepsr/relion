...
void ColorHelper::writeSignedToEPS(
    std::string filename, int col, const std::vector<Image<RFLOAT> > &imgs,
    ...
    RFLOAT width_ratio = width / 595.0;
    RFLOAT height_ratio = height / 792.0;
    RFLOAT max_ratio = std::max(width_ratio, height_ratio);
    RFLOAT rescale = std::max(max_ratio, 1.0);
