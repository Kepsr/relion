...
void FilterHelper::drawTestPattern(Image<RFLOAT> &img, int squareSize) {
    ...
    for (long int x = 0; x < img.data.xdim; x++) {
        int xi = x / squareSize % 2;
        int yi = y / squareSize % 2;
...
void FilterHelper::separableGaussianXY(
    const Image<RFLOAT> &src, Image<RFLOAT> &dest,
    ...
    for (size_t x = 0; x < src.data.xdim; x++) {
        ...
        for (long int i = -k; i <= k; i++) {
            ...
            if (wrap) { 
                xx = (xx + src.data.xdim) % src.data.xdim; 
            } else if (xx < 0 || xx >= src.data.xdim) {
                continue;
            }
    ...
    for (size_t x = 0; x < src.data.xdim; x++) {
        ...
        for (long int i = -k; i <= k; i++) {
            ...
            if (wrap) { 
                yy = (yy + src.data.ydim) % src.data.ydim; 
            } else if (yy < 0 || yy >= temp.data.ydim) {
                continue;
            }
...
void FilterHelper::separableGaussianX_wrap(
    const Image<RFLOAT> &src, const Image<RFLOAT> &mask, Image<RFLOAT> &dest, RFLOAT sigma, int k
) {
    if (k < 0) { k = 2 * sigma + 0.5; }
...
void FilterHelper::separableGaussianX_wrap(
    const Image<RFLOAT> &src, Image<RFLOAT> &dest, RFLOAT sigma, int k
) {
    if (k < 0) { k = 2 * sigma + 0.5; }
...
void FilterHelper::averageX(
    const Image<RFLOAT> &src, const Image<RFLOAT> &mask, Image<RFLOAT> &dest
) {
    dest.data.resize(src.data);
