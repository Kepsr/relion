...
Image<RFLOAT> ConvolutionHelper::convolve2D(
    Image<RFLOAT> &img0, Image<RFLOAT> &img1, FourierTransformer &ft
) {
    int w = img0().xdim;
    int wf = w / 2 + 1;
    int h = img0().ydim;
...
Image<RFLOAT> ConvolutionHelper::gaussianKernel2D(
    double sigma, int w, int h, bool normalize, bool centered, bool half
) {
    Image<RFLOAT> out(w, h);
