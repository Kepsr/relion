...
motionParamEstimator(), motionEstimator(), frameRecombiner() {}
...
void MotionRefiner::read(int argc, char **argv) {
    ...
    if (
        micrographHandler.eer_upsampling > 0 || 
        micrographHandler.eer_grouping > 0
    ) {
...
void MotionRefiner::init() {
    ...
    if (minMG >= allMdts.size()) {
        ...
        REPORT_ERROR(
            "ERROR: Cannot start with micrograph " + sts0.str()
    ...
    */
    ...
    recombineFrames = frameRecombiner.doingRecombination() && (
        recombMdts.size() > 0 || 
        !exists(outPath + "shiny" + frameRecombiner.getOutputSuffix() + ".star")
    );
...
void MotionRefiner::combineEPSAndSTARfiles() {
    ...
    for (long g = 0; g < allMdts.size(); g++) {
        ...
        if (
            frameRecombiner.doingRecombination() && 
            exists(fn_root + "_shiny" + frameRecombiner.getOutputSuffix() + ".star")
        ) {
            MetaDataTable mdt;
    ...
    if (frameRecombiner.doingRecombination()) {
        for (int og = 0; og < obsModel.numberOfOpticsGroups(); og++) {
            ...
            if (n_OgPresent[og] == 0) {
                ...
                obsModel.opticsMdt.setValue(EMDL::IMAGE_PIXEL_SIZE, -1.0, og); // mark for deletion
                continue;
            }
            ...
            obsModel.opticsMdt.setValue(EMDL::IMAGE_PIXEL_SIZE, frameRecombiner.getOutputPixelSize(og), og);
            obsModel.opticsMdt.setValue(EMDL::IMAGE_SIZE, frameRecombiner.getOutputBoxSize(og), og);
            obsModel.opticsMdt.setValue(EMDL::OPTIMISER_DATA_ARE_CTF_PREMULTIPLIED, frameRecombiner.isCtfMultiplied(og), og);
        ...
        while (og < obsModel.opticsMdt.numberOfObjects()) {
            RFLOAT og_angpix;
            obsModel.opticsMdt.getValue(EMDL::IMAGE_PIXEL_SIZE, og_angpix, og);
            if (og_angpix < 0) {
...
FileName MotionRefiner::getOutputFileNameRoot(
    ...
    mdt.getValue(EMDL::MICROGRAPH_NAME, fn_mic, 0);
