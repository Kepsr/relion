...
void MotionHelper::noiseNormalize(
    ...
    for (int x = 0; x < wf; x++) {
        ...
        dest(y, x) = r >= wf || sigma2[r] == 0.0 ? 
            Complex(0.0) :
            DIRECT_A2D_ELEM(img.data, y, x) / sqrt(sigma2[r] * area);
    }
}
...
std::vector<std::vector<d2Vector>> MotionHelper::readTracksInPix(
    std::string fn, double angpix
) {
    std::ifstream ifs(fn);
    ...
    for (int p = 0; p < pc; p++) {
        ...
        for (int f = 0; f < fc; f++) {
            mdt.getValue(EMDL::ORIENT_ORIGIN_X_ANGSTROM, out[p][f].x, f);
            mdt.getValue(EMDL::ORIENT_ORIGIN_Y_ANGSTROM, out[p][f].y, f);
