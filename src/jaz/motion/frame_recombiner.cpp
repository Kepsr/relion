...
void FrameRecombiner::init(
    ...
    */
    ...
    for (int og = 0; og < nog; og++) {
        ...
        freqWeights[og] = hasBfacs ?
            weightsFromBfacs(allMdts, s_out[og], angpix_out[og]) :
            weightsFromFCC  (allMdts, s_out[og], angpix_out[og], sts.str());
    }
}

void FrameRecombiner::process(
...
//			std::cout << "OUT xoff = " << xoff << " yoff = " << yoff << " xcoord = " << xcoord << " ycoord = " << ycoord << std::endl;;
        ...
        for (int p = 0; p < pc; p++) {
            std::stringstream sts;
            sts << (p + 1);
            mdtOut.setValue(EMDL::IMAGE_NAME, sts.str() + "@" + fn_root+"_shiny" + suffix + ".mrcs", p);
        }
...
std::vector<Image<RFLOAT>> FrameRecombiner::weightsFromFCC(
    ...
    for (int x = 0; x < sh_ref; x++) {
        ...
        fcc(y, x) = wgh > 0.0 ? fccData(y, x) / wgh : 0.0;
    }
...
int FrameRecombiner::getOutputBoxSize(int opticsGroup) {
    return crop_arg > 0 ? crop_arg : s_out[opticsGroup];
}
...
bool FrameRecombiner::isJobFinished(std::string filenameRoot) {
    return (
        exists(filenameRoot + "_shiny" + suffix + ".mrcs") && 
