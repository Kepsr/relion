...
MotionEstimator::MotionEstimator(): paramsRead(false), ready(false) {}
...
void MotionEstimator::init(
    ...
    for (int g = 0; g < obsModel->numberOfOpticsGroups(); g++) {
        ...
        /// TODO: make filter flank width (around FSC=0.143) a parameter (and greater than 3 pixels)
        for (int f = 0; f < fc; f++) {
...
void MotionEstimator::process(
    const std::vector<MetaDataTable>& mdts, long g_start, long g_end
) {
    if (!ready) {
    ...
    for (long g = g_start; g <= g_end; g++) {
        ...
        if (debug) {
            std::string mgName = mdts[g].getValue<std::string>(EMDL::MICROGRAPH_NAME, 0);
...
void MotionEstimator::prepMicrograph(
    ...
    for (int p = 0; p < pc; p++) {
        positions[p].x = mdt.getValue<double>(EMDL::IMAGE_COORD_X, p);
        positions[p].y = mdt.getValue<double>(EMDL::IMAGE_COORD_Y, p);
    }

    movie = micrographHandler->loadMovie(
        mdt, s[ogmg], angpix[ogmg], fts,
        positions, myInitialTracks, unregGlob, myGlobComp
    ); // throws exceptions
...
//	std::cout << "motion estimator preds size = " << XSIZE(preds[0]()) << "x" << YSIZE(preds[0]()) << std::endl;
    ...
    if (global_init || myInitialTracks.size() == 0) {
        ...
        if (diag) ImageLog::write(
            ccSum, MotionRefiner::getOutputFileNameRoot(outPath, mdt) + "_CCsum", CenterXY
        );
        ...
        for (int p = 0; p < pc; p++) {
            ...
            for (int f = 0; f < fc; f++) {
                myInitialTracks[p][f] = unregGlob ? globOffsets[p] : globTrack[f] + globOffsets[p];
            }
        }
        ...
        globOffsets = MotionHelper::getGlobalOffsets(
            movieCC, myInitialTracks, cc_pad, 0.25 * s[ogmg], 
            globOffMax, globOffMax, nr_omp_threads
        );
...
std::vector<std::vector<d2Vector>> MotionEstimator::optimize(
    ...
    GpMotionFit gpmf(
        movieCC, cc_pad, sig_vel_px, sig_div_px, sig_acc_px,
...
std::vector<std::vector<d2Vector>> MotionEstimator::optimize(
    ...
    return optimize(
        CCd, inTracks, sig_vel_px, sig_acc_px, sig_div_px, positions, globComp
    );
}

std::vector<Image<RFLOAT>> MotionEstimator::computeDamageWeights(int opticsGroup) {
    return DamageHelper::damageWeights(
        s[opticsGroup], angpix[opticsGroup], micrographHandler->firstFrame, 
        fc, dosePerFrame, dmga, dmgb, dmgc
    );
}
...
void MotionEstimator::updateFCC(
    ...
    for (int p = 0; p < pc; p++) {
        ...
        FscHelper::updateFscTable(
            obs, pred, scale,
            tables[threadnum], weights0[threadnum], weights1[threadnum]
        );
    }
}
...
void MotionEstimator::writeOutput(
    ...
    for (int f = 0; f < fc; f++) {
        CDataPoint point(
            xcenterCoord + visScale * globalTrack[f].x,
...
void MotionEstimator::proposeDosePerFrame(double dpf, std::string metaFn, int verb) {
    if (dosePerFrame < 0) {
        if (metaFn == "") {
            REPORT_ERROR_STR(
                "ERROR: No electron dose available. Please provide one "
...
bool MotionEstimator::isJobFinished(std::string filenameRoot) {
    return (
        exists(filenameRoot + "_tracks.star") && 
