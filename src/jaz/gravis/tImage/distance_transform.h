 ...
 * WARNING: This file is not part of any build target.
 * 
 */
...
namespace gravis {
    ...
    static inline void intensityThreshold(
        tImage<bool> &out, const tImage<T> &in, const F &threshold = 0
    ) {
        out.resize(in.cols(), in.rows());
        for (size_t i = 0; i < in.size(); ++i) {
            out[i] = in[i].intensity() > threshold;
        }
    };
    ...
    static inline
    void alphaThreshold(
        tImage<bool> &out, const tImage<T> &in, 
        const F &threshold = typename tImageTraits<T>::Float_t(0)
    ) {
        out.resize(in.cols(), in.rows());
        for (size_t i = 0; i < in.size(); ++i) { 
            out[i] = in[i].a > threshold; 
        }
    };
    ...
    inline static void distanceTransform(
        tImage<F> &out, const tImage<Pixel> &in, 
        const int &w = 3, const size_t &iterations = 3
    ) {
        const int W = in.cols();
        const int H = in.rows();
