        ...
        if (localOpt) {
            if (rigidLocalOpt) {
                ...
                for (int f = 0; f < fc-1; f++) {
                    double sv = f < sig_vel_vec.size() ? sig_vel_vec[f] : 
                        sig_vel_vec[sig_vel_vec.size() - 1];
            ...
            if (!rigidLocalOptOnly) {
                ...
                for (int f = 0; f < fc - 1; f++) {
                    ...
                    for (int p = 0; p < pc; p++) {
                        ...
                        for (int q = 0; q < pc; q++) {
                            ...
                            double sd = f < sig_div_vec.size() ? sig_div_vec[f] : 
                                sig_div_vec[sig_div_vec.size() - 1];
                ...
                LocalMotionFit lmf(
                    motEM.initialCC, velWgh, accWgh, divWgh,
                    std::vector<d2Vector>(fc, d2Vector(0,0)), nr_omp_threads
                );
        ...
        if (evaluate) {
            if (measureFCC) {
                ...
                for (int p = 0; p < pc; p++) {
                    ...
                    FscHelper::updateFscTableVelWgh(
                        obs, vel, pred, tablesVW[threadnum],
        ...
        if (it_number > 0) {
            ...
            for (int p = 0; p < pc; p++) {
                ...
                for (int f = 0; f < fc; f++) {
                    ...
                    if (f < 15) {
                        visOut15 << visTracks[p][f].x << " " << visTracks[p][f].y << "\n";
                    }
                }
