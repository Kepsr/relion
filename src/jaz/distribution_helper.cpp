...
Image<double> DistributionHelper::drawHistogramStack(
    const std::vector<std::vector<double>> &data, int height
) {
    const int fc = data.size();
    const int bins = data[0].size();

    Image<double> hist = Image<double>::zeros(bins, height, fc);
    hist.data.xinit = 0;
    hist.data.yinit = 0;
    hist.data.zinit = 0;
...
std::vector<double> DistributionHelper::normalizeRing(
    const std::vector<double> &histogram, double step
) {
    ...
    if (sum > 0.0) {
        for (int i = 0; i < s; i++) { out[i] = histogram[i] / sum; }
    }
...
std::vector<double> DistributionHelper::toEnergy(const std::vector<double> &probability) {
    ...
    for (int i = 0; i < s; i++) { out[i] = -log(probability[i]); }
...
std::vector<double> DistributionHelper::toProbability(const std::vector<double> &energy) {
    ...
    for (int i = 0; i < s; i++) { out[i] = exp(-energy[i]); }

    return out;
}

void DistributionHelper::writeRing(
    const std::vector<double> &values, double step,
...
std::vector<double> DistributionHelper::bivariateGaussStudentProbability(
    ...
    for (int i = 0; i < bins; i++) {
        double x = (i + 0.5) * step;
        out[i] = theta / twoPiSigmaGSq * exp(-0.5 * x * x / (sigmaG * sigmaG))
                  + (1.0 - theta) / twoPiSigmaSSq * pow(1.0 + x * x / nuSigmaSSq, -(1.0 + nu / 2.0));
    }
...
std::vector<double> DistributionHelper::doubleGaussProbability(
    ...
    for (int i = 0; i < bins; i++) {
        double x = (i + 0.5) * step;
        out[i] = theta * rho + (1.0 - theta) * (1.0 / twoPiSigmaSq) * exp(-0.5 * x * x / sig2);
    }
...
std::vector<double> DistributionHelper::fitMultiStudent(
    ...
    for (int i = 0; i < pbc; i++) {
        initial[2 * i + 2] = 3.0; // nu
        initial[2 * i + 3] = 0.2; // theta
    }
...
CorruptedExponentialFit::CorruptedExponentialFit(const std::vector<double> &velocities, double area): velocities(velocities), area(area) {}
...
BivariateStudentFit::BivariateStudentFit(const std::vector<double> &values, double fixedNu): values(values), fixedNu(fixedNu) {}
...
CorruptedBivariateStudentFit::CorruptedBivariateStudentFit(
    const std::vector<double> &values, double area, double fixedNu
): values(values), area(area), fixedNu(fixedNu) {}

double CorruptedBivariateStudentFit::f(const std::vector<double> &x, void* tempStorage) const {

    const double sigma = x[0];
    const double nu = fixedNu > 0.0 ? fixedNu : x[1];
    const double theta = x[fixedNu > 0.0 ? 1 : 2];
...
GaussStudentFit::GaussStudentFit(const std::vector<double> &values, double fixedNu): values(values), fixedNu(fixedNu) {}
...
DoubleStudentFit::DoubleStudentFit(
    const std::vector<double> &values, const std::vector<int> &counts, double nu0, bool fixedNu0, double nuS, bool fixedNuS, double area, bool relTheta
): values(values), counts(counts), myNu0(nu0), myNuS(nuS), area(area), fixedNu0(fixedNu0), fixedNuS(fixedNuS), relTheta(relTheta) {}
...
MultiStudentFit::MultiStudentFit(
    const std::vector<double> &values,
    const std::vector<int> &counts, int minPN,
    int maxPN, double area
): values(values), counts(counts), minPN(minPN), maxPN(maxPN), area(area) {}
...
DoubleGaussFit::DoubleGaussFit(
    const std::vector<double> &values,
    const std::vector<int> &counts, const std::vector<int> &totalCounts,
    double area,
    bool centered
): values(values), counts(counts), totalCounts(totalCounts), area(area), centered(centered) {}
...
CorruptedGaussFit::CorruptedGaussFit(const std::vector<double> &values, double area): values(values), area(area) {}
