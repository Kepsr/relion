...
class GaussianPyramid {
    ...
    GaussianPyramid(Image<T> img, double sigma = 1.0,
...
template <typename T>
GaussianPyramid<T>::GaussianPyramid(Image<T> img, double sigma, double cutoff, int maxLev):
    ...
    while (i < maxLev) {
        ...
        Image<T> filt, next;
        FilterHelper::separableGaussianXY(img, filt, dSigma, 3*dSigma, true);
...
void GaussianPyramid<T>::interpolationTest(const Image<T> &img) {
    ...
    for (double s0 = 0.0; s0 < 2.0; s0 += ds) {
        ...
        std::ofstream ofs("debug/interp_" + std::to_string(s0) + "-" + std::to_string(s1) + ".dat");
...
void GaussianPyramid<T>::timeTest(const Image<T> &img0) {
    ...
    for (int p = 0; p < pc;     p++)
    for (int q = 0; q < pc - 1; q++) {
        fs.data = ft.FourierTransform(img.data);
        ...
        for (int i = 0; i < sh; i++) {
            ...
            fs(i, j) *= exp(-0.5 * (x * x + y * y) / sh2);
        }

        ft.inverseFourierTransform(fs.data, rs.data);
