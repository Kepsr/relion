...
void DamageHelper::fitDamage(
    const Image<RFLOAT> &frcData, const Image<RFLOAT> &frcWeight,
...
Image<RFLOAT> DamageHelper::plotDamage(
    const std::vector<double> &amp, const std::vector<double> &dec, int frames
) {
...
Image<RFLOAT> DamageHelper::plotDamageFrc(
    const std::vector<double> &amp, const std::vector<double> &dec, int frames
) {
    Image<RFLOAT> out(amp.size(), frames);
...
void DamageHelper::fitGlobalDamage(
...
Image<RFLOAT> DamageHelper::plotGlobalDamage(
    double a, double b, double c,
...
Image<RFLOAT> DamageHelper::plotGlobalDamage(
    double a, double b, double c, int freqs, int frames,
    double angpix, double dosePerFrame, bool frc
) {
...
std::vector<double> DamageHelper::fitBFactors(
    ...
    if (verb > 0) { std::cout << "best f: " << bestF << "\n"; }
    ...
    for (int it = 0; it < 5; it++) {
        ...
        for (int k = 0; k < kc; k++) {
            ...
            const double eps = 1e-20;
            scale[k] = dividend / std::max(divisor, eps);
        }
    }
...
std::pair<std::vector<d2Vector>, std::vector<double>> DamageHelper::fitBkFactors(
    ...
    for (int k = 0; k < kc; k++) {
        scale[k] = std::max(0.0, fcc(bestF,k));
    }
...
std::vector<d2Vector> DamageHelper::fitBkFactors(
    ...
    for (int f = 0; f < fc; f++) {
        for (int k = 0; k < kc; k++) {
            envelope[k] = std::max(0.0, env(f,k));
            weight  [k] = std::max(0.0, wgh(f,k));
        }
...
d2Vector DamageHelper::findSigmaKRec(
    ...
    for (int s = 0; s < steps; s++) {
        ...
        const double eps = 1e-20;
        double a = num / std::max(denom, eps);
...
template <typename T>
static void normalise(std::vector<Image<T>> &images, const int kc, const int kc2, const int fc) {
    for (int y = 0; y < kc2; y++)
    for (int x = 0; x < kc;  x++) {
...
std::vector<Image<RFLOAT>> DamageHelper::computeWeights(
    ...
    if (normalize) normalise(out, kc, kc2, fc);
...
GlobalDamageFit::GlobalDamageFit(
    const Image<RFLOAT> &snrData, const Image<RFLOAT> &snrWeight, 
    int k0, int k1, int t0, bool L1
): snrData(snrData), snrWeight(snrWeight), k0(k0), k1(k1), t0(t0), L1(L1) {}
...
PerFrameBFactorFit::PerFrameBFactorFit(
    const Image<RFLOAT> &fcc, int k0, int k1
): fcc(fcc), kc(fcc.data.xdim), fc(fcc.data.ydim), k0(k0), k1(k1) {}
