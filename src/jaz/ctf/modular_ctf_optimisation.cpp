...
double ModularCtfOptimisation::f(const std::vector<double> &x) const {
    ...
    for (int p = 0; p < particle_count; p++) {
        ...
        for (int xi = 0; xi < sh; xi++) {
            ...
            const double yy = (yi <= s / 2 ? yi : yi - s) / as;
...
void ModularCtfOptimisation::grad(
    ...
    for (int p = 0; p < particle_count; p++) {
        ...
        for (int xi = 0; xi < sh; xi++) {
            ...
            if (modes[Defocus] != Fixed) {
                (*out)[
                    t * stride + paramOffset[Defocus] + 
                    p * paramParticleStep[Defocus]
                ] += dE_dDz;
            }

            if (modes[Astigmatism1] != Fixed) {
                (*out)[
                    t * stride + paramOffset[Astigmatism1] + 
                    p * paramParticleStep[Astigmatism1]
                ] += dE_dA1;
            }

            if (modes[Astigmatism2] != Fixed) {
                (*out)[
                    t * stride + paramOffset[Astigmatism2] + 
                    p * paramParticleStep[Astigmatism2]
                ] += dE_dA2;
            }

            if (modes[SphericalAberration] != Fixed) {
                (*out)[
                    t * stride + paramOffset[SphericalAberration] + 
                    p * paramParticleStep[SphericalAberration]
                ] += dE_dCs;
            }

            if (modes[BFactor] != Fixed) {
                (*out)[
                    t * stride + paramOffset[BFactor] + 
                    p * paramParticleStep[BFactor]
                ] += dE_dBf;
            }

            if (modes[ScaleFactor] != Fixed) {
                (*out)[
                    t * stride + paramOffset[ScaleFactor] + 
                    p * paramParticleStep[ScaleFactor]
                ] += dE_dKf;
            }
        }
    }
...
void ModularCtfOptimisation::writeToTable(const std::vector<double> &x) {
    for (int p = 0; p < particle_count; p++) {
        ...
        mdt.setValue(EMDL::CTF_DEFOCUS_ANGLE, angleDeg, p);

        // cs = K[2] = (PI / 2) * C_s * 1e7 * lambda^3
        // => C_s = cs * (2/PI) / (1e7 / lambda^3)
        if (modes[SphericalAberration] != Fixed)
            mdt.setValue(EMDL::CTF_CS, 2 * cs / (1e7 * PI * lambda * lambda * lambda), p);

        if (modes[BFactor] != Fixed)
            mdt.setValue(EMDL::CTF_BFACTOR, bf, p);

        if (modes[ScaleFactor] != Fixed)
            mdt.setValue(EMDL::CTF_SCALEFACTOR, kf, p);
    }
}
