...
void CtfRefiner::read(int argc, char **argv) {
    ...
    only_do_unfinished = parser.checkOption(
        "--only_do_unfinished",
        "Skip those steps for which output files already exist."
    );

    do_ctf_padding = parser.checkOption(
        "--ctf_pad",
        "Use larger box to calculate CTF and then downscale to mimic boxing operation in real space"
    );
    diag = parser.checkOption("--diag", "Write out diagnostic data (slower)");

    int fit_section = parser.addSection("Defocus fit options");
    do_defocus_fit = parser.checkOption(
        "--fit_defocus",
    ...
    do_bfac_fit = parser.checkOption("--fit_bfacs", "Estimate CTF B-factors");
    ...
    do_mag_fit = parser.checkOption(
        "--fit_aniso", "Estimate anisotropic magnification"
    );
...
void CtfRefiner::init() {
    ...
    if (!ObservationModel::containsAllColumnsNeededForPrediction(mdt0)) {
        REPORT_ERROR_STR(
            starFn << " does not contain all columns needed for view prediction: \n"
...
void CtfRefiner::processSubsetMicrographs(long g_start, long g_end) {
    ...
    for (long g = g_start; g <= g_end; g++) {
        ...
        if (do_mag_fit) {
            ...
            magnificationEstimator.processMicrograph(
                g, unfinishedMdts[g], obs, predOppT, predGradient, do_ctf_padding
            );
        }
...
void CtfRefiner::finalise() {
    ...
    mdtOut = do_bfac_fit || do_defocus_fit ? merge(allMdts, fn_eps_later) : allMdts;
...
std::vector<MetaDataTable> CtfRefiner::merge(
    const std::vector<MetaDataTable>& mdts, std::vector<FileName> &fn_eps
) {
    int gc = mdts.size();
    int barstep;
