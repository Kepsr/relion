...
DefocusEstimator::DefocusEstimator(): ready(false) {}

void DefocusEstimator::read(IOParser &parser, int argc, char *argv[]) {

    fittingMode = parser.getOption(
        "--fit_mode",
...
void DefocusEstimator::bruteForceFit(
    ...
    for (long p = 0; p < pc; p++) {
        ...
        if (fitAstigmatism) {
            ...
            mdt.setValue(EMDL::CTF_DEFOCUS_ANGLE, phi, p);
        } else {
            double u, v;
