...
void MagnificationEstimator::read(IOParser &parser, int argc, char *argv[]) {
    kmin = textToFloat(parser.getOption("--kmin_mag", "Inner freq. threshold for anisotropic magnification estimation [Angst]", "20.0"));
...
void MagnificationEstimator::processMicrograph(
    ...
    if (!ready) {
        REPORT_ERROR_STR(
            "ERROR: MagnificationEstimator::processMicrograph: "
    ...
    for (int pog = 0; pog < particlesByOpticsGroup.size(); pog++) {
        ...
        /// TODO: SHWS 29 Mar 2018: when data is CTF-premultiplied: do we need to change updateScaleFreq??
...
void MagnificationEstimator::parametricFit(
    ...
    if (!ready) {
        REPORT_ERROR_STR(
            "ERROR: MagnificationEstimator::parametricFit: "
            << "MagnificationEstimator not initialized."
        );
    }
    ...
    bool hasMagMatrices = optOut.labelExists(EMDL::IMAGE_MAG_MATRIX_00)
                       && optOut.labelExists(EMDL::IMAGE_MAG_MATRIX_01)
                       && optOut.labelExists(EMDL::IMAGE_MAG_MATRIX_10)
                       && optOut.labelExists(EMDL::IMAGE_MAG_MATRIX_11);
    ...
    for (int og = 0; og < ogc; og++) {
        ...
        #pragma omp critical
        {
            ...
            optOut.setValue(EMDL::IMAGE_MAG_MATRIX_11, mat1(1, 1), og);
    ...
    if (adaptAstig) {
        MagnificationHelper::adaptAstigmatism(
            mat_by_optGroup, mdts, !perMgAstig, obsModel
        );
    }
}
