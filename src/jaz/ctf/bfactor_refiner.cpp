...
BFactorRefiner::BFactorRefiner(): ready(false) {}

void BFactorRefiner::read(IOParser &parser, int argc, char *argv[]) {
    perMicrograph = parser.checkOption(
        "--bfac_per_mg",
        "Estimate B-factors per micrograph, instead of per particle"
    );

    min_B = textToDouble(parser.getOption(
        "--bfac_min_B", "Minimal allowed B-factor", "-30"
    ));

    max_B = textToDouble(parser.getOption(
        "--bfac_max_B", "Maximal allowed B-factor", "300"
    ));

    min_scale = textToDouble(parser.getOption(
        "--bfac_min_scale",
        "Minimal allowed scale-factor (essential for outlier rejection)", "0.2"
    ));

    kmin = textToDouble(parser.getOption(
        "--kmin_bfac",
...
void BFactorRefiner::processMicrograph(
    ...
    if (perMicrograph) {
        ...
        for (long p = 0; p < pc; p++) {
            ...
            ctfImg() = ctf.getFftwImage(sh[og], s[og], s[og], s[og], angpix[og], false, false, false, false, do_ctf_padding);
        ...
        d2Vector BK = BFactorRefiner::findBKRec1D(
            t_rad[0], s_rad[0], min_B_px[ogRef], max_B_px[ogRef], 
            min_scale, stepsPerIter, numIters
        );

        for (long p = 0; p < pc; p++) {
            mdt.setValue(EMDL::CTF_BFACTOR, as[ogRef] * as[ogRef] * BK[0] - min_B, p);
            mdt.setValue(EMDL::CTF_SCALEFACTOR, BK[1], p);
        }
        ...
        for (int t = 0; t < nr_omp_threads; t++) {
            for (int i = 0; i < valsPerPart[t].size(); i++) {
                ...
                mdt.setValue(EMDL::CTF_BFACTOR, as[og] * as[og] * BK[0] - min_B, p);
                mdt.setValue(EMDL::CTF_SCALEFACTOR, BK[1], p);
            }
        }
...
void BFactorRefiner::writePerMicrographEPS(
    ...
    RFLOAT B = mdt.getValue<RFLOAT>(EMDL::CTF_BFACTOR,     0);
    RFLOAT a = mdt.getValue<RFLOAT>(EMDL::CTF_SCALEFACTOR, 0);
...
void BFactorRefiner::writePerParticleEPS(const MetaDataTable& mdt) {
    ...
    for (int p = 0; p < pc; p++) {
        ...
        RFLOAT B     = mdt.getValue<RFLOAT>(EMDL::CTF_BFACTOR, p);
        RFLOAT a     = mdt.getValue<RFLOAT>(EMDL::CTF_SCALEFACTOR, p);
...
d2Vector BFactorRefiner::findBKRec2D(
    ...
    for (int st = 0; st < steps; st++) {
        ...
        for (long x = 0; x < sh; x++) {
            ...
            Complex vx = direct::elem(pred.data, x, y);
            const Complex vy = direct::elem(obs.data, x, y);
            const double vw = direct::elem(weight.data, x, y);
        ...
        double a = num / (denom > eps ? denom : eps);
