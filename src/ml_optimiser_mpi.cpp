...
void MlOptimiserMpi::initialise() {
    ...
    if (node->isLeader()) {
        ...
        bool do_warn = false;
        for (int igroup = 0; igroup< mymodel.nr_groups; igroup++) {
            if (mymodel.nr_particles_per_group[igroup] < 5 && node->rank == 1) {
                    ...
                    if (wsum_model.BPref[ith_recons].weight.sum() > XMIPP_EQUAL_ACCURACY) {
                        ...
                        if (do_external_reconstruct) {
                            ...
                            wsum_model.BPref[ith_recons].externalReconstruct(
                                ...
                                mymodel.tau2_fudge_factor,
                                node->rank == 1
                            ); // only first follower is verbose
                        } else {
...
void MlOptimiserMpi::iterate() {
    ...
    for (iter = iter + 1; iter <= nr_iter; iter++) {
        ...
        if (node->isLeader()) {
            if (
                do_helical_refine && !do_skip_align && !do_skip_rotate && mymodel.ref_dim == 3
            ) {
