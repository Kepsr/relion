...
// enum {
//     OUTPUT_MEAN_MAP_ONLY = 1,
//     OUTPUT_STDDEV_MAP_ONLY,
//     OUTPUT_BOTH_MEAN_AND_STDDEV_MAPS
// }
...
class ccfPixel {
    ...
    ccfPixel(): x(-1.0), y(-1.0), fom(-1.0) {}; // psi(-1.0)
    ccfPixel(RFLOAT x, RFLOAT y, RFLOAT fom): x(x), y(y), fom(fom) {}; // psi(psi)
...
class AutoPicker {
    ...
    #ifdef TIMING
    Timer timer;
    /// TODO: Don't hard-code the number of timing labels
    ...
    AutoPicker(): 
    available_memory(0), available_gpu_memory(0), requested_gpu_memory(0)
    {}
