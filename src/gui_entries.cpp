...
void GuiEntry::initialise(
    int x, int y, 
    Fl_Group* deactivate_this_group, bool _actually_activate,
...
void GuiEntry::place(
    JobOption &_joboption, int &y, 
    int _deactivate_option, Fl_Group* deactivate_this_group, bool actually_activate, 
...
void GuiEntry::cb_browse_i() {
    ...
    Fl::scheme("gtk+");
    Fl_File_Chooser * G_chooser = new Fl_File_Chooser(
        "", joboption.pattern.c_str(), Fl_File_Chooser::SINGLE, ""
    );

    G_chooser->directory((
        joboption.directory == "CURRENT_ODIR" ?
        current_browse_directory : joboption.directory
    ).c_str());
    ...
    if (!create_scheduler_gui) {
        ...
        if (my_deactivate_group) {
            ...
            if (std::string(inp->value()).substr(0, 2) == "$$") {
                ...
                actually_activate && (strcmp(inp->value(), "Yes") == 0) || 
