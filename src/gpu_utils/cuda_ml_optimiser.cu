...
void getFourierTransformsAndCtfs(
    long int my_ori_particle,
    OptimisationParamters &op,
    SamplingParameters &sp,
    MlOptimiser *baseMLO,
    MlOptimiserCuda *cudaMLO
) {
    GTICTOC(cudaMLO->timer, "getFourierTransformsAndCtfs", ({
    ...
    for (int ipart = 0; ipart < baseMLO->mydata.ori_particles[my_ori_particle].particles_id.size(); ipart++) {
        ...
        if (
            ...
            bool do_classification_local_searches = !baseMLO->do_auto_refine && baseMLO->mymodel.orientational_prior_mode == PRIOR_ROTTILT_PSI
                    && baseMLO->mymodel.sigma2_rot > 0.0 && baseMLO->mymodel.sigma2_tilt > 0.0 && baseMLO->mymodel.sigma2_psi > 0.0;
            ...
            if (prior_rot > 998.99 && prior_rot < 999.01)
                prior_rot = direct::elem(baseMLO->exp_metadata, op.metadata_offset + ipart, METADATA_ROT);
            if (prior_tilt > 998.99 && prior_tilt < 999.01)
                prior_tilt = direct::elem(baseMLO->exp_metadata, op.metadata_offset + ipart, METADATA_TILT);
            ...
            if (prior_psi > 998.99 && prior_psi < 999.01)
                prior_psi = direct::elem(baseMLO->exp_metadata,op. metadata_offset + ipart, METADATA_PSI);
            ...
            if (baseMLO->do_helical_refine) {
                baseMLO->sampling.selectOrientationsWithNonZeroPriorProbabilityFor3DHelicalReconstruction(
        ...
        CTICTOC(cudaMLO->timer, "setXmippOrigin1", ({
        ...
        if (baseMLO->do_parallel_disc_io) {
            ...
            if (baseMLO->do_preread_images) {
                ...
                if (cudaMLO->dataIs3D) {
                    CTICTOC(cudaMLO->timer, "ParaRead3DImages", ({
                    img.read(fn_img);
                    img().setXmippOrigin();
                    }))
                } else {
                    CTICTOC(cudaMLO->timer, "ParaRead2DImages", ({
            ...
            if (baseMLO->has_converged && baseMLO->do_use_reconstruct_images) {
                ...
                for (int i = 0; i <= istop; i++) { getline(split2, fn_recimg); }
            ...
            if (cudaMLO->dataIs3D) {
                CTICTOC(cudaMLO->timer, "Read3DImages", ({
                CTICTOC(cudaMLO->timer, "resize", ({
        ...
        CTICTOC(cudaMLO->timer, "selfTranslate", ({

        /** FIXME:
         * For some reason the device allocation inside "selfTranslate" takes much longer than expected.
        ...
        for (int i = 0; i < img_size; i++) { temp[i] = img.data.data[i]; }
        ...
        if (baseMLO->do_norm_correction) {
            CTICTOC(cudaMLO->timer, "norm_corr", ({
        ...
        if (baseMLO->has_converged && baseMLO->do_use_reconstruct_images) {
            // rec_img is NOT norm_corrected in the CPU code, so neither is it here.
        ...
        CTICTOC(cudaMLO->timer, "calcFimg", ({
        ...
        int FMultiBsize = ceilf((float) cudaMLO->transformer1.fouriers.getSize() * 2 / (float) BLOCK_SIZE);
        ...
        CTICTOC(cudaMLO->timer, "cpFimg2Host", ({
        d_Fimg.cp_to_host();
        d_Fimg.streamSync();
        ...
        CTICTOC(cudaMLO->timer, "selfApplyBeamTilt", ({
        ...
        CTICTOC(cudaMLO->timer, "zeroMask", ({
        MultidimArray<RFLOAT> Mnoise;
        bool is_helical_segment = baseMLO->do_helical_refine || baseMLO->mymodel.ref_dim == 2 && baseMLO->helical_tube_outer_diameter > 0.0;
        ...
        if (!baseMLO->do_zero_mask) {
            ...
            init_random_generator(baseMLO->random_seed + (baseMLO->do_realign_movies ? part_id : my_ori_particle));
            // my_ori_particle: This only serves for exact reproducibility tests with 1.3 code.
            ...
            CTICTOC(cudaMLO->timer, "inverseFourierTransform", ({
            cudaMLO->transformer.inverseFourierTransform();
            }))

            CTICTOC(cudaMLO->timer, "setXmippOrigin2", ({
            Mnoise.setXmippOrigin();
            }))

            CTICTOC(cudaMLO->timer, "softMaskOutsideMap", ({
            d_img.cp_to_host();
            d_img.streamSync();
            ...
            for (int i = 0; i < img_size; i++) { d_img[i] = img.data.data[i]; }
            d_img.cp_to_device();
            }))
            ...
            XFLOAT radius = (RFLOAT) baseMLO->particle_diameter / (2.0 * baseMLO-> mymodel.pixel_size);
            if (radius < 0) { radius = (RFLOAT) img.data.xdim / 2.0; }
        ...
        CTICTOC(cudaMLO->timer, "transform", ({
        d_img.cp_on_device(cudaMLO->transformer1.reals);

        runCenterFFT(
            // runs on input GlobalPtr.stream
            cudaMLO->transformer1.reals,
            (int) cudaMLO->transformer1.xSize,
            (int) cudaMLO->transformer1.ySize,
            (int) cudaMLO->transformer1.zSize,
        ...
        int FMultiBsize2 = ceilf((float) cudaMLO->transformer1.fouriers.getSize() * 2 / (float) BLOCK_SIZE);
        ...
        CTICTOC(cudaMLO->timer, "powerClass", ({
        ...
        if (baseMLO->mymodel.current_size < baseMLO->mymodel.ori_size) {
            CudaGlobalPtr<XFLOAT> spectrumAndXi2(
                baseMLO->mymodel.ori_size / 2 + 2, 0, cudaMLO->devBundle->allocator
            ); // last +1 is the Xi2, to remove an expensive memcpy
        ...
        // Also store its CTF
        CTICTOC(cudaMLO->timer, "ctfCorr", ({
        CTICTOC(cudaMLO->timer, "cpFimg2Host_2", ({
        d_Fimg.streamSync();
        d_Fimg.cp_to_host();
        d_Fimg.streamSync();
        for (int i = 0; i < Fimg.size(); i ++) {
        ...
        if (baseMLO->do_ctf_correction) {
            if (cudaMLO->dataIs3D) {
                ...
                CTICTOC(cudaMLO->timer, "CTFSet3D_array", ({
                Ictf().setXmippOrigin();
                ...
                CTF ctf = CTF(
                    direct::elem(baseMLO->exp_metadata, op.metadata_offset + ipart, METADATA_CTF_DEFOCUS_U),
                    direct::elem(baseMLO->exp_metadata, op.metadata_offset + ipart, METADATA_CTF_DEFOCUS_V),
                    direct::elem(baseMLO->exp_metadata, op.metadata_offset + ipart, METADATA_CTF_DEFOCUS_ANGLE),
                    direct::elem(baseMLO->exp_metadata, op.metadata_offset + ipart, METADATA_CTF_VOLTAGE),
                    direct::elem(baseMLO->exp_metadata, op.metadata_offset + ipart, METADATA_CTF_CS),
                    direct::elem(baseMLO->exp_metadata, op.metadata_offset + ipart, METADATA_CTF_Q0),
                    direct::elem(baseMLO->exp_metadata, op.metadata_offset + ipart, METADATA_CTF_BFAC),
                    1.0,
                    direct::elem(baseMLO->exp_metadata, op.metadata_offset + ipart, METADATA_CTF_PHASE_SHIFT)
                );

                ctf.getFftwImage(
                    Fctf, baseMLO->mymodel.ori_size, baseMLO->mymodel.ori_size, baseMLO->mymodel.pixel_size,
...
void getAllSquaredDifferencesCoarse(
    ...
    CTICTOC(cudaMLO->timer, "diff_pre_gpu", ({
    ...
    baseMLO->precalculateShiftedImagesCtfsAndInvSigma2s(
        false, op.my_ori_particle, sp.current_image_size, sp.current_oversampling, op.metadata_offset, // inserted SHWS 12112015
    ...
    if (cudaMLO->devBundle->generateProjectionPlanOnTheFly) {
        CTICTOC(cudaMLO->timer, "generateProjectionSetupCoarse", ({
        ...
        for (int iclass = sp.iclass_min; iclass <= sp.iclass_max; iclass++) {
            if (baseMLO->mymodel.pdf_class[iclass] > 0.0) {
                projectorPlans[iclass].setup(
                    baseMLO->sampling,
                    op.directions_prior, op.psi_prior,
                    op.pointer_dir_nonzeroprior, op.pointer_psi_nonzeroprior,
                    NULL, // Mcoarse_significant
                    baseMLO->mymodel.pdf_class,
                    baseMLO->mymodel.pdf_direction,
                    sp.nr_dir, sp.nr_psi,
                    sp.idir_min, sp.idir_max,
                    sp.ipsi_min, sp.ipsi_max,
                    sp.itrans_min, sp.itrans_max,
                    0,  // current_oversampling
                    1,  // nr_oversampled_rot
                    iclass,
                    true, // coarse
                    !IS_NOT_INV,
                    baseMLO->do_skip_align, baseMLO->do_skip_rotate, baseMLO->mymodel.orientational_prior_mode
                );
            }
        }
        }))
    ...
    for (long int ipart = 0; ipart < sp.nr_particles; ipart++) {
        ...
        CTICTOC(cudaMLO->timer, "translation_1", ({
        ...
        deviceInitValue(allWeights, (XFLOAT) (op.highres_Xi2_imgs[ipart] / 2.0));
        allWeights_pos = 0;
...
void getAllSquaredDifferencesFine(
    ...
    CTICTOC(cudaMLO->timer, "precalculateShiftedImagesCtfsAndInvSigma2s", ({
    std::vector<MultidimArray<Complex> > dummy;
    baseMLO->precalculateShiftedImagesCtfsAndInvSigma2s(
        false, op.my_ori_particle, sp.current_image_size, sp.current_oversampling, op.metadata_offset, // inserted SHWS 12112015
        sp.itrans_min, sp.itrans_max, op.Fimgs, dummy, op.Fctfs, dummy, dummy,
    ...
    for (long int ipart = 0; ipart < sp.nr_particles; ipart++) {
        ...
        CTICTOC(cudaMLO->timer, "translation_2", ({
        ...
        CTICTOC(cudaMLO->timer, "kernel_init_1", ({
        ...
        for (int exp_iclass = sp.iclass_min; exp_iclass <= sp.iclass_max; exp_iclass++) {
            ...
            if (baseMLO->mymodel.pdf_class[exp_iclass] > 0.0 && FineProjectionData[ipart].class_entries[exp_iclass] > 0) {
                ...
                CTICTOC(cudaMLO->timer, "pair_list_1", ({
                ...
                int chunkSize = 0;
                if (cudaMLO->dataIs3D || cudaMLO->refIs3D) {
                ...
                significant_num = makeJobsForDiff2Fine(
                    op,	sp,  // A lot of different type inputs
                ...
                CTICTOC(cudaMLO->timer, "IndexedArrayMemCp2", ({
                ...
                CTICTOC(cudaMLO->timer, "generateEulerMatrices", ({
        ...
        for (int exp_iclass = sp.iclass_min; exp_iclass <= sp.iclass_max; exp_iclass++) {
            if (baseMLO->mymodel.pdf_class[exp_iclass] > 0.0 && FineProjectionData[ipart].class_entries[exp_iclass] > 0) {
                ...
                if (orientation_num == 0) continue;

                long unsigned significant_num(FPCMasks[ipart][exp_iclass].weightNum);
                if (significant_num == 0) continue;

                CTICTOC(cudaMLO->timer, "Diff2MakeKernel", ({
                CudaProjectorKernel projKernel = CudaProjectorKernel::makeKernel(
                    cudaMLO->devBundle->cudaProjectors[exp_iclass],
                    op.local_Minvsigma2s[0].xdim,
                    op.local_Minvsigma2s[0].ydim,
                ...
                IndexedDataArray thisClassFinePassWeights(
                    FinePassWeights[ipart],FPCMasks[ipart][exp_iclass], cudaMLO->devBundle->allocator
                );

                CTICTOC(cudaMLO->timer, "Diff2CALL", ({
                ...
                runDiff2KernelFine(
                    ...
                    ~thisClassFinePassWeights.rot_id, ~thisClassFinePassWeights.rot_idx, ~thisClassFinePassWeights.trans_idx,
        ...
        CTICTOC(cudaMLO->timer, "collect_data_1", ({
...
void convertAllSquaredDifferencesToWeights(
    ...
    #ifdef TIMING
    if (op.my_ori_particle == baseMLO->exp_my_first_ori_particle) {
        baseMLO->timer.tic(exp_ipass == 0 ? baseMLO->TIMING_ESP_WEIGHT1 : baseMLO->TIMING_ESP_WEIGHT2);
    }
    #endif
    ...
    CTICTOC(cudaMLO->timer, "get_orient_priors", ({
    for (int exp_iclass = sp.iclass_min; exp_iclass <= sp.iclass_max; exp_iclass++)
    ...
    op.significant_weight.resize(sp.nr_particles, 0.0);
    ...
    for (long int ipart = 0; ipart < sp.nr_particles; ipart++) {
        ...
        if (baseMLO->do_cc()) {
            ...
            for (int exp_iclass = sp.iclass_min; exp_iclass <= sp.iclass_max; exp_iclass++) {
                sumRedSize += ceilf((float) (exp_ipass == 0 ? sp.nr_dir * sp.nr_psi : FPCMasks[ipart][exp_iclass].jobNum) / (float) SUMW_BLOCK_SIZE);
            }
            ...
            CTICTOC(cudaMLO->timer, "get_offset_priors", ({
            ...
            CTICTOC(cudaMLO->timer, "sumweight1", ({
            ...
            if (exp_ipass == 0) {
                ...
                if (sizeof(weights_t) == sizeof(XFLOAT)) {
                    ...
                    // BUG: This looks like an ill-formed function call.
                ...
                if (ipart_length > 1) {
                    ...
                    CudaGlobalPtr<weights_t> unsorted_ipart(
                        weights,
                        ipart * op.Mweight.xdim + sp.nr_dir * sp.nr_psi * sp.nr_trans * sp.iclass_min,
                        ipart_length
                    );
                    ...
                    MoreThanCubOpt<weights_t> moreThanOpt(0.0);
                    size_t filteredSize = filterOnDevice(unsorted_ipart, filtered, moreThanOpt);

                    if (filteredSize == 0) {
                        if (failsafeMode) {
                            // Only print error if not managed to recover through fail-safe mode
                            std::cerr << std::endl;
                    ...
                    if (
                        baseMLO->maximum_significants > 0 &&
                    ...
                    std::pair<int, weights_t> max_pair;
                    CTICTOC(cudaMLO->timer, "getArgMaxOnDevice", ({
                    max_pair = getArgMaxOnDevice(unsorted_ipart);
                    }))
                    op.max_index[ipart].coarseIdx = max_pair.first;
                    op.max_weight[ipart] = max_pair.second;

                    // Store nr_significant_coarse_samples for this particle
                    direct::elem(baseMLO->exp_metadata, op.metadata_offset + ipart, METADATA_NR_SIGN) = (RFLOAT) my_nr_significant_coarse_samples;
                ...
                for (int exp_iclass = sp.iclass_min; exp_iclass <= sp.iclass_max; exp_iclass++) {
                    ...
                    if (baseMLO->mymodel.pdf_class[exp_iclass] > 0.0 && FPCMasks[ipart][exp_iclass].weightNum > 0) {
                        ...
                        cuda_kernel_exponentiate_weights_fine<<<block_dim,SUMW_BLOCK_SIZE, 0, cudaMLO->classStreams[exp_iclass]>>>(
                            ~pdf_orientation_class, ~pdf_offset_class,
                            ~thisClassPassWeights.weights,
                            (XFLOAT) local_norm,
                            sp.nr_oversampled_rot, sp.nr_oversampled_trans,
                            ~thisClassPassWeights.rot_id, ~thisClassPassWeights.trans_idx,
                            ~FPCMasks[ipart][exp_iclass].jobOrigin,
                            ~FPCMasks[ipart][exp_iclass].jobExtent,
                ...
                if (baseMLO->adaptive_oversampling != 0) {
                    ...
                    std::pair<int, XFLOAT> max_pair;
                    CTICTOC(cudaMLO->timer, "getArgMaxOnDevice", ({
                    max_pair = getArgMaxOnDevice(PassWeights[ipart].weights);
                    }))
                    op.max_index[ipart].fineIdx = PassWeights[ipart].ihidden_overs[max_pair.first];
                    op.max_weight[ipart] = max_pair.second;
                } else {
    ...
    #ifdef TIMING
    if (op.my_ori_particle == baseMLO->exp_my_first_ori_particle) {
        baseMLO->timer.toc(exp_ipass == 0 ? baseMLO->TIMING_ESP_WEIGHT1 : baseMLO->TIMING_ESP_WEIGHT2);
    }
    #endif
}
...
void storeWeightedSums(
    ...
    CTICTOC(cudaMLO->timer, "store_init", ({
    ...
    for (long int ipart = 0; ipart < sp.nr_particles; ipart++) {
        long int part_id = baseMLO->mydata.ori_particles[op.my_ori_particle].particles_id[ipart];
        int group_id = baseMLO->mydata.getGroupId(part_id);
        op.local_Minvsigma2s[ipart][0] = 1.0 / (baseMLO->sigma2_fudge * direct::elem(baseMLO->mymodel.sigma2_noise[group_id], 0));
    }
    ...
    exp_wsum_norm_correction.resize(sp.nr_particles, 0.0);
    ...
    thr_wsum_pdf_class.resize(baseMLO->mymodel.nr_classes, 0.0);
    if (baseMLO->mymodel.ref_dim == 2) {
        thr_wsum_prior_offsetx_class.resize(baseMLO->mymodel.nr_classes, 0.0);
    ...
    for (long int ipart = 0; ipart < sp.nr_particles; ipart++) {
        ...
        long int part_id = baseMLO->mydata.ori_particles[op.my_ori_particle].particles_id[ipart];
        int group_id = baseMLO->mydata.getGroupId(part_id);
        CTICTOC(cudaMLO->timer, "collect_data_2_pre_kernel", ({
        for (int exp_iclass = sp.iclass_min; exp_iclass <= sp.iclass_max; exp_iclass++) {
        ...
        for (int exp_iclass = sp.iclass_min; exp_iclass <= sp.iclass_max; exp_iclass++) {
            ...
            runCollect2jobs(
                ...
                sp.nr_trans,
                sp.nr_oversampled_trans, sp.nr_oversampled_rot,
                oversamples,
                baseMLO->do_skip_align || baseMLO->do_skip_rotate,
        ...
        CTICTOC(cudaMLO->timer, "collect_data_2_post_kernel", ({
    ...
    for (long int ipart = 0; ipart < sp.nr_particles; ipart++) {
        CTICTOC(cudaMLO->timer, "setMetadata", ({
        ...
        // TODO We already have rot, tilt and psi. Don't calculate them again.
        if (baseMLO->do_skip_align || baseMLO->do_skip_rotate) {
            baseMLO->sampling.getOrientations(
                sp.idir_min, sp.ipsi_min, baseMLO->adaptive_oversampling, oversampled_rot, oversampled_tilt, oversampled_psi,
                op.pointer_dir_nonzeroprior, op.directions_prior, op.pointer_psi_nonzeroprior, op.psi_prior
            );
        } else {
            baseMLO->sampling.getOrientations(
                op.max_index[ipart].idir, op.max_index[ipart].ipsi, baseMLO->adaptive_oversampling, oversampled_rot, oversampled_tilt, oversampled_psi,
                op.pointer_dir_nonzeroprior, op.directions_prior, op.pointer_psi_nonzeroprior, op.psi_prior
            );
        }
        baseMLO->sampling.getOrientations(
        ...
        // Grab rot / tilt / psi for later use before updating.
        RFLOAT old_rot = direct::elem(baseMLO->exp_metadata, op.metadata_offset + ipart, icol_rot);
        direct::elem(baseMLO->exp_metadata, op.metadata_offset + ipart, icol_rot) = rot;
        RFLOAT old_tilt = direct::elem(baseMLO->exp_metadata, op.metadata_offset + ipart, icol_tilt);
        direct::elem(baseMLO->exp_metadata, op.metadata_offset + ipart, icol_tilt) = tilt;
        ...
        direct::elem(baseMLO->exp_metadata, op.metadata_offset + ipart, icol_xoff) = XX(shifts);
        direct::elem(baseMLO->exp_metadata, op.metadata_offset + ipart, icol_yoff) = YY(shifts);
        if (cudaMLO->dataIs3D)
        direct::elem(baseMLO->exp_metadata, op.metadata_offset + ipart, icol_zoff) = ZZ(shifts);

        if (ibody == 0) {
            direct::elem(baseMLO->exp_metadata, op.metadata_offset + ipart, METADATA_CLASS) = (RFLOAT) op.max_index[ipart].iclass + 1;
            ...
            direct::elem(baseMLO->exp_metadata, op.metadata_offset + ipart, METADATA_PMAX) = pmax;
        }
        }))
    }
    }))
    ...
    CTICTOC(cudaMLO->timer, "maximization", ({
    ...
    for (long int ipart = 0; ipart < sp.nr_particles; ipart++) {
        ...
        CTICTOC(cudaMLO->timer, "translation_3", ({
        ...
        for (int exp_iclass = sp.iclass_min; exp_iclass <= sp.iclass_max; exp_iclass++) {
            if (
                baseMLO->mymodel.pdf_class[exp_iclass] == 0.0 || ProjectionData[ipart].class_entries[exp_iclass] == 0
            ) continue;

            // Use the constructed mask to construct a partial class-specific input
            IndexedDataArray thisClassFinePassWeights(FinePassWeights[ipart],FPCMasks[ipart][exp_iclass], cudaMLO->devBundle->allocator);

            CTICTOC(cudaMLO->timer, "thisClassProjectionSetupCoarse", ({
            // use "slice" constructor with class-specific parameters to retrieve a temporary ProjectionParams with data for this class
            ProjectionParams thisClassProjectionData(
                ProjectionData[ipart],
            ...
            CTICTOC(cudaMLO->timer, "generateEulerMatricesProjector", ({
            ...
            CTICTOC(cudaMLO->timer, "pre_wavg_map", ({

            for (long unsigned i = 0; i < orientation_num * translation_num; i++)
                sorted_weights[classPos + i] = -999.0;
        ...
        for (int exp_iclass = sp.iclass_min; exp_iclass <= sp.iclass_max; exp_iclass++) {
            ...
            runWavgKernel(
                ...
                ~ctfs,
                ~wdiff2s_sum,
                &wdiff2s_AA(AAXA_pos), &wdiff2s_XA(AAXA_pos),
                op,
                orientation_num, translation_num,
                image_size,
                ipart,
                ...
                cudaMLO->dataIs3D, cudaMLO->classStreams[exp_iclass]
            );
            ...
            CTICTOC(cudaMLO->timer, "backproject", ({
            ...
            runBackProjectKernel(
                ...
                (XFLOAT) op.sum_weight[ipart],
                ~eulers[exp_iclass],
                op.local_Minvsigma2s[0].xdim, op.local_Minvsigma2s[0].ydim, op.local_Minvsigma2s[0].zdim,
                orientation_num,
                cudaMLO->dataIs3D,
                baseMLO->do_sgd,
                cudaMLO->classStreams[exp_iclass]
        ...
        for (int exp_iclass = sp.iclass_min; exp_iclass <= sp.iclass_max; exp_iclass++) {
            ...
            for (long int j = 0; j < image_size; j++) {
                ...
                if (
                    ...
                    direct::elem(exp_wsum_scale_correction_XA[ipart], ires) += wdiff2s_XA[AAXA_pos + j];
                }
            }
            AAXA_pos += image_size;
        }
    ...
    CTICTOC(cudaMLO->timer, "store_post_gpu", ({
    ...
    for (long int ipart = 0; ipart < sp.nr_particles; ipart++) {
        ...
        for (int ires = baseMLO->mymodel.current_size / 2 + 1; ires < baseMLO->mymodel.ori_size / 2 + 1; ires++) {
            direct::elem(thr_wsum_sigma2_noise[group_id], ires) += direct::elem(op.power_imgs[ipart], ires);
            // Also extend the weighted sum of the norm_correction
            exp_wsum_norm_correction[ipart] += direct::elem(op.power_imgs[ipart], ires);
        }

        // Store norm_correction
        // Multiply by old value because the old norm_correction term was already applied to the image
        if (baseMLO->do_norm_correction) {
            RFLOAT old_norm_correction = direct::elem(baseMLO->exp_metadata, op.metadata_offset + ipart, METADATA_NORM);
            ...
            RFLOAT normcorr = old_norm_correction * sqrt(exp_wsum_norm_correction[ipart] * 2.0);
            thr_avg_norm_correction += normcorr;

            // Now set the new norm_correction in the relevant position of exp_metadata
            direct::elem(baseMLO->exp_metadata, op.metadata_offset + ipart, METADATA_NORM) = normcorr;


            // Print warning for strange norm-correction values
            if (!baseMLO->do_cc() && direct::elem(baseMLO->exp_metadata, op.metadata_offset + ipart, METADATA_NORM) > 10.0) {
                std::cout << " WARNING: norm_correction= "<< direct::elem(baseMLO->exp_metadata, op.metadata_offset + ipart, METADATA_NORM)
        ...
        RFLOAT dLL = baseMLO->do_cc() ? -op.min_diff2[ipart] : log(op.sum_weight[ipart]) - local_norm - logsigma2;

        // Store dLL of each image in the output array, and keep track of total sum
        direct::elem(baseMLO->exp_metadata, op.metadata_offset + ipart, METADATA_DLL) = dLL;
        thr_sum_dLL += dLL;

        // Also store sum of Pmax
        thr_sum_Pmax += direct::elem(baseMLO->exp_metadata, op.metadata_offset + ipart, METADATA_PMAX);
...
size_t MlDeviceBundle::checkFixedSizedObjects(int shares) {
    ...
    if (device_id >= devCount) CRITICAL(ERR_GPUID);
    ...
    size_t BytesNeeded = (float) shares * margin * 2.5 * sizeof(XFLOAT) * pow(2 * baseMLO->mymodel.ori_size, 3);
    ...
    for (int iclass = 0; iclass < nr_classes; iclass++) {
        ...
        if (!generateProjectionPlanOnTheFly && baseMLO->mymodel.pdf_class[iclass] > 0.0) {
            ...
            coarseProjectionPlans[iclass].setup(
                baseMLO->sampling,
                exp_directions_prior, exp_psi_prior,
                exp_pointer_dir_nonzeroprior, exp_pointer_psi_nonzeroprior,
                NULL, //Mcoarse_significant
                baseMLO->mymodel.pdf_class, baseMLO->mymodel.pdf_direction,
                nr_idir, nr_ipsi,
                0, //idir_min
                nr_idir - 1, //idir_max
                0, //ipsi_min
...
void MlOptimiserCuda::doThreadExpectationSomeParticles(int thread_id) {
    ...
    #endif
    // CTICTOC(cudaMLO->timer, "interParticle", ({
        ...
        for (long unsigned ipart = first_ipart; ipart <= last_ipart; ipart++) {
            ...
            if (baseMLO->do_generate_seeds) {
                if (baseMLO->do_firstiter_cc && baseMLO->iter == 1) {
                    ...
                    baseMLO->iter == 1 && !baseMLO->do_firstiter_cc ||
                    baseMLO->iter == 2 &&  baseMLO->do_firstiter_cc
                ) {
            ...
            op.significant_weight.resize(sp.nr_particles, -1.0);
            ...
            for (int ipass = 0; ipass < nr_sampling_passes; ipass++) {
                ...
                if (ipass == 0) {
                    ...
                    deviceInitValue<XFLOAT>(Mweight, -999.0);
                    Mweight.streamSync();
