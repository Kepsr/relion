...
void getOrientations(
    HealpixSampling &sampling, long int idir, long int ipsi, int oversampling_order,
...
void CudaProjectorPlan::setup(
    ...
    if (sampling.is_3D) {
        if (abs(sampling.random_perturbation) > 0.0) {
            ...
            cuda_kernel_make_eulers_3D<inverseMatrix, false><<<grid_size, BLOCK_SIZE, 0, eulers.getStream()>>>(
                ~alphas, ~betas, ~gammas, ~eulers, orientation_num, NULL
            );
        }
    } else {
        cuda_kernel_make_eulers_2D<inverseMatrix><<<grid_size, BLOCK_SIZE, 0, eulers.getStream()>>>(
