/**
 * @file cuda_utils_cub.cuh
 * 
 * @warning Currently not a part of any build target.
 * 
 */
...
static std::pair<int, T> getArgMinOnDevice(CudaGlobalPtr<T> &ptr) {
    ...
    return std::make_pair<int, T>(min_pair[0].key, min_pair[0].value);
}
...
class AllocatorThrustWrapper {
    ...
    AllocatorThrustWrapper(CudaCustomAllocator *allocator): allocator(allocator) {}
