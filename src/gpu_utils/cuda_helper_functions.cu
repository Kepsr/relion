...
long int makeJobsForDiff2Fine(
    ...
    for (long unsigned i = 0; i < orientation_num; i++) {
        ...
        for (long unsigned j = 0; j < translation_num; j++) {
            ...
            if (direct::elem(op.Mcoarse_significant, ipart, ihidden) == 1) {
        ...
        for (long int ipsi = sp.ipsi_min, ipart = 0; ipsi <= sp.ipsi_max; ipsi++, iorient++) {
            ...
            if (baseMLO->isSignificantAnyParticleAnyTranslation(iorientclass, sp.itrans_min, sp.itrans_max, op.Mcoarse_significant)) {
                ...
                baseMLO->sampling.getOrientations(
                    idir, ipsi, sp.current_oversampling,
...
// So much repetition inside this function!
void runWavgKernel(
    ...
    unsigned image_size,
    long int ipart, int group_id, int exp_iclass,
    XFLOAT part_scale,
    bool refs_are_ctf_corrected, bool data_is_3D,
    cudaStream_t stream
) {
    ...
    if (refs_are_ctf_corrected) {
        if (data_is_3D) {
            cuda_kernel_wavg<true, true, true, WAVG_BLOCK_SIZE_DATA3D>
            <<<block_dim, WAVG_BLOCK_SIZE_DATA3D, (3 * WAVG_BLOCK_SIZE_DATA3D + 9) * sizeof(XFLOAT), stream>>>
            (
            ...
            cuda_kernel_wavg<true, true, false, WAVG_BLOCK_SIZE>
            <<<block_dim, WAVG_BLOCK_SIZE, (3 * WAVG_BLOCK_SIZE + 9) * sizeof(XFLOAT), stream>>>
            (
                eulers, projector, image_size, orientation_num,
                Fimg_real, Fimg_imag,
                trans_x, trans_y, trans_z,
            ...
            cuda_kernel_wavg<true, false, false, WAVG_BLOCK_SIZE>
            <<<block_dim, WAVG_BLOCK_SIZE, (3 * WAVG_BLOCK_SIZE + 9) * sizeof(XFLOAT), stream>>>
            (
                eulers, projector, image_size, orientation_num,
                Fimg_real, Fimg_imag,
        ...
        if (data_is_3D) {
            cuda_kernel_wavg<false, true, true, WAVG_BLOCK_SIZE_DATA3D>
            <<<block_dim, WAVG_BLOCK_SIZE_DATA3D, (3 * WAVG_BLOCK_SIZE_DATA3D + 9) * sizeof(XFLOAT), stream>>>
            (
                eulers, projector, image_size, orientation_num,
                Fimg_real, Fimg_imag,
            ...
            cuda_kernel_wavg<false, true, false, WAVG_BLOCK_SIZE>
            <<<block_dim, WAVG_BLOCK_SIZE, (3 * WAVG_BLOCK_SIZE + 9) * sizeof(XFLOAT), stream>>>
            (
                eulers, projector, image_size, orientation_num,
                Fimg_real, Fimg_imag,
            ...
            cuda_kernel_wavg<false, false, false, WAVG_BLOCK_SIZE>
            <<<block_dim, WAVG_BLOCK_SIZE, (3 * WAVG_BLOCK_SIZE + 9) * sizeof(XFLOAT), stream>>>
            (
                eulers, projector, image_size, orientation_num,
                Fimg_real, Fimg_imag,
...
void runBackProjectKernel(
    ...
    if (BP.mdlZ == 1) {
        ...
        if (do_sgd) {
            if (data_is_3D){
                cuda_kernel_backprojectSGD<true>
                ...
                (
                    ...
                    trans_x, trans_y, trans_z,
                    d_weights, d_Minvsigma2s, d_ctfs,
                    translation_num, significant_weight, weight_norm,
                    d_eulers,
                    BP.d_mdlReal, BP.d_mdlImag, BP.d_mdlWeight,
                    BP.maxR, BP.maxR2,
                    BP.padding_factor,
                    ...
                    imgX * imgY * imgZ,
                    BP.mdlX, BP.mdlY,
                    BP.mdlInitY, BP.mdlInitZ
                );
            } else{
                cuda_kernel_backprojectSGD<false>
            ...
            if (data_is_3D) {
                cuda_kernel_backproject3D<true>
                <<<imageCount, BP_DATA3D_BLOCK_SIZE, 0, optStream>>>
                (
                    d_img_real, d_img_imag,
                    trans_x, trans_y, trans_z,
                ...
                cuda_kernel_backproject3D<false>
                <<<imageCount, BP_REF3D_BLOCK_SIZE, 0, optStream>>>
                (
                    d_img_real, d_img_imag,
                    trans_x, trans_y, trans_z,
...
void runDiff2KernelCoarse(
    ...
    long unsigned orientation_num, int translation_num, int image_size,
    cudaStream_t stream,
    ...
    if (!do_CC) {
            if (data_is_3D) {
                ...
                (
                    ...
                    diff2s, image_size,
                    op.highres_Xi2_imgs[ipart] / 2.0,
                    orientation_num, translation_num,
                    job_num_count, //significant_num,
                    rot_idx, trans_idx, job_idx, job_num
                );
            } else if (projector.mdlZ != 0) {
        ...
        if (data_is_3D) {
            ...
            (
                ...
                Fimgs_real, Fimgs_imag,
                trans_x, trans_y, trans_z,
                projector, corr_img, diff2s, image_size,
                op.highres_Xi2_imgs[ipart] / 2.0,
...
void selfApplyBeamTilt2(
    ...
    RFLOAT wavelength, RFLOAT Cs, RFLOAT angpix, 
    int ori_size
) {
    if (Fimg.getDim() != 2)
