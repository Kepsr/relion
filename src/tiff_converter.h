...
class TIFFConverter {
    ...
    template <typename T>
    static void write_tiff_one_page(
        TIFF *tif, MultidimArray<T> buf, const float pixel_size=-1, const int filter=COMPRESSION_LZW, const int level=6, const bool strip_per_line=false
    ) {
