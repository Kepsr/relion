...
int main(int argc, char *argv[]) {
    ...
    for (int m = 0; m < sourceByMic.size(); m++) {
        std::string micName;
        sourceByMic[m].getValue(EMDL::MICROGRAPH_NAME, micName, 0);
    ...
    for (int m = 0; m < refByMic.size(); m++) {
        std::string micName;
        refByMic[m].getValue(EMDL::MICROGRAPH_NAME, micName, 0);
        ...
        for (int p = 0; p < pcSrc; p++) {
            src->getValue(EMDL::IMAGE_COORD_X, posSrc[p].x, p);
            src->getValue(EMDL::IMAGE_COORD_Y, posSrc[p].y, p);
        }
        
        std::vector<d2Vector> posRef(pcRef);
        
        for (int p = 0; p < pcRef; p++) {
            refByMic[m].getValue(EMDL::IMAGE_COORD_X, posRef[p].x, p);
            refByMic[m].getValue(EMDL::IMAGE_COORD_Y, posRef[p].y, p);
        }
        ...
        for (int p = 0; p < pcRef; p++) {
            ...
            if (qBest >= 0) {
                ...
                int randSubsetSrc;
                src->getValue(EMDL::PARTICLE_RANDOM_SUBSET, randSubsetSrc, qBest);
                
                int randSubsetRef;
                refByMic[m].getValue(EMDL::PARTICLE_RANDOM_SUBSET, randSubsetRef, p);
                ...
                if (copyAngles) {
                    ...
                    refByMic[m].getValue(EMDL::ORIENT_ROT, rot, p);
                    refByMic[m].getValue(EMDL::ORIENT_TILT, tilt, p);
                    refByMic[m].getValue(EMDL::ORIENT_PSI, psi, p);
                ...
                if (copyOffsets) {
                    ...
                    refByMic[m].getValue(EMDL::ORIENT_ORIGIN_X, xoff, p);
                    refByMic[m].getValue(EMDL::ORIENT_ORIGIN_Y, yoff, p);
