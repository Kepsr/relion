...
class helix_bilder_parameters {
    ...
    void read(int argc, char **argv) {
        ...
        if (rise_min_A    > rise_max_A)    std::swap(rise_min_A,    rise_max_A);
        if (twist_min_deg > twist_max_deg) std::swap(twist_min_deg, twist_max_deg);
    ...
    void run() {
        ...
        valid_options += (int) do_check_parameters;
        valid_options += (int) do_normalise_segments;
        valid_options += (int) do_interpo_3D_curve;
        valid_options += (int) do_select_3D_subtomo_from_2D_proj;
        valid_options += (int) do_average_au_2d;
        valid_options += (int) do_debug;
        ...
        if (do_extract_coords_relion || do_extract_coords_ximdisp || do_extract_coords_eman) {
            ...
            extractHelicalSegmentsFromTubes_Multiple(
                fn_in_root, fn_out_root, format_tag, nr_asu,
                rise_A, pixel_size_A, Xdim, Ydim, boxdim,
                do_bimodal_searches, do_cut_into_segments
            );
            ...
            convertHelicalSegmentCoordsToStarFile_Multiple(
                fn_in_root, fn_out_root, format_tag,
                pixel_size_A, Xdim, Ydim, boxdim,
                do_bimodal_searches
            );
            ...
            combineParticlePriorsWithKaiLocalCTF_Multiple(
                fn_in1_root, fn_in2_root, fn_out_root
            );
            ...
            createCylindricalReferenceWithPolarity(
                ...
                topbottom_ratio, width_edge_pix
            );
            applySoftSphericalMask(
                img(), RFLOAT(boxdim) * sphere_percentage, width_edge_pix
            );
            const long int i = img.header.size() - 1;
            img.header.setValue(EMDL::IMAGE_SAMPLINGRATE_X, pixel_size_A, i);
            img.header.setValue(EMDL::IMAGE_SAMPLINGRATE_Y, pixel_size_A, i);
            img.header.setValue(EMDL::IMAGE_SAMPLINGRATE_Z, pixel_size_A, i);
            img.write(fn_out);
            ...
            setNullTiltPriorsInDataStar(fn_in, fn_out);
            ...
            removeBadTiltHelicalSegmentsFromDataStar(fn_in, fn_out, tilt_max_dev_deg);
            ...
            excludeLowCTFCCMicrographs(
                fn_in, fn_out, ctf_fom_min, EPA_lowest_res, df_min, df_max
            );
            ...
            makeHelicalReference3DWithPolarity(
                img(),
                boxdim, pixel_size_A, twist_deg, rise_A,
                cyl_outer_diameter_A, subunit_diameter_A,
                (do_polar_reference ? cyl_inner_diameter_A : subunit_diameter_A),
                (do_polar_reference ? topbottom_ratio : 1.0),
                sym_Cn, nr_filaments_helix_with_seam
            );
            applySoftSphericalMask(
                img(), RFLOAT(boxdim) * sphere_percentage, width_edge_pix
            );
            ...
            localSearchHelicalSymmetry(
                ...
                sphere_diameter_A / 2.0,
                cyl_inner_diameter_A / 2.0, cyl_outer_diameter_A / 2.0,
                z_percentage, rise_min_A, rise_max_A, rise_inistep_A, rise_refined_A,
                twist_min_deg, twist_max_deg, twist_inistep_deg, twist_refined_deg,
                (verb ? (&std::cout) : NULL)
            );
            ...
            if (cyl_outer_diameter_A < 0.0)
                cyl_outer_diameter_A = 0.0;
                /// TODO: PLEASE CHECK THIS FOR OTHER OPTIONS!

            Assembly pdb_ori, pdb_helix;
            pdb_ori.readPDB(fn_in);
            makeSimpleHelixFromPDBParticle(
                pdb_ori, pdb_helix, cyl_outer_diameter_A / 2.0,
                twist_deg, rise_A, nr_subunits,
            ...
            simulateHelicalSegments(
                is_3d_tomo, fn_in, fn_out, white_noise, boxdim,
                nr_subunits, nr_asu, nr_tubes, do_bimodal_searches,
                cyl_outer_diameter_A, pixel_size_A, rise_A, twist_deg,
                sigma_psi, sigma_tilt, sigma_offset, random_seed
            );
            ...
            bool result = checkParametersFor3DHelicalReconstruction(
                ignore_helical_symmetry, do_helical_symmetry_local_refinement, nr_asu,
                rise_A, rise_min_A, rise_max_A, twist_deg, twist_min_deg, twist_max_deg,
                boxdim, pixel_size_A, z_percentage,
                sphere_percentage * boxdim * pixel_size_A,
                cyl_inner_diameter_A, cyl_outer_diameter_A, true // verbose
            );
            if (result)
            ...
            normaliseHelicalSegments(
                fn_in, fn_out_root, cyl_outer_diameter_A, pixel_size_A
            );
            ...
            Interpolate3DCurves(
                fn_in_root, fn_out_root, nr_asu, rise_A, pixel_size_A,
                boxdim, binning_factor, do_bimodal_searches
            );
