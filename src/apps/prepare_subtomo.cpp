...
class prepare_subtomo {
    ...
    void initialChecks() {
        ...
        if (do_use_only_lower_tilt_defoci) {
            if (lower_tilt_defoci_limit <= 0.0)
        ...
        FOR_ALL_OBJECTS_IN_METADATA_TABLE(MD_tomo) {
            ...
            if (exists(fn3)) {
                ...
                if (
                    !MD_this_tomo.containsLabel(EMDL::IMAGE_COORD_X) || 
                    !MD_this_tomo.containsLabel(EMDL::IMAGE_COORD_Y) || 
                    !MD_this_tomo.containsLabel(EMDL::IMAGE_COORD_Z)
    ...
    void run() {
        ...
        MD_part.clear();
        MD_part.addLabel(EMDL::MICROGRAPH_NAME);
        MD_part.addLabel(EMDL::IMAGE_COORD_X);
        MD_part.addLabel(EMDL::IMAGE_COORD_Y);
        MD_part.addLabel(EMDL::IMAGE_COORD_Z);
        MD_part.addLabel(EMDL::IMAGE_NAME);
        MD_part.addLabel(EMDL::CTF_IMAGE);
        MD_part.addLabel(EMDL::CTF_MAGNIFICATION);
        MD_part.addLabel(EMDL::CTF_DETECTOR_PIXEL_SIZE);
        ...
        FOR_ALL_OBJECTS_IN_METADATA_TABLE(MD_tomo) {
            ...
            MD_ctf.addLabel(EMDL::MICROGRAPH_NAME);
            ...
            if (
                !MD_ctf_results.containsLabel(EMDL::CTF_DEFOCUSU) || 
            ...
            if (do_use_only_lower_tilt_defoci) {
                RFLOAT sum_defoci = 0.0, nr_defoci = 0.0;
            ...
            if (fn_coords.getExtension() != "star") {
                MD_coords.addLabel(EMDL::IMAGE_COORD_X);
                MD_coords.addLabel(EMDL::IMAGE_COORD_Y);
                MD_coords.addLabel(EMDL::IMAGE_COORD_Z);
            ...
            FOR_ALL_OBJECTS_IN_METADATA_TABLE(MD_coords) {
                ...
                MD_this_subtomo.addLabel(EMDL::CTF_DEFOCUSU);
                MD_this_subtomo.addLabel(EMDL::CTF_VOLTAGE);
                MD_this_subtomo.addLabel(EMDL::CTF_CS);
                MD_this_subtomo.addLabel(EMDL::CTF_Q0);
                MD_this_subtomo.addLabel(EMDL::ORIENT_ROT);
                MD_this_subtomo.addLabel(EMDL::ORIENT_TILT);
                MD_this_subtomo.addLabel(EMDL::ORIENT_PSI);
                MD_this_subtomo.addLabel(EMDL::CTF_BFACTOR);
                MD_this_subtomo.addLabel(EMDL::CTF_SCALEFACTOR);

                // Find minimum and maximum tilts
                RFLOAT min_tilt = 999999.0, max_tilt = -999999.0;
                for (int id = 0; id < tilts.size(); id++) {
