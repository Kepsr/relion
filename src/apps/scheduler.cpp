...
class scheduler_parameters {
    ...
    void usage() {
        ...
        std::cerr << "    --schedule test --add operator --type " << Schedule::FLOAT_OPERATOR_PLUS << " --i iter --i2 iter_step --o iter" << std::endl;
        std::cerr << "    --schedule test --add operator --type " << Schedule::FLOAT_OPERATOR_PLUS << " --i iter --i2 1 --o iter" << std::endl;
        std::cerr << "    --schedule test --add operator --type " << Schedule::OPERATOR_TOUCH_FILE << " --i initial_model" << std::endl;
        std::cerr << "    --schedule test --add operator --type " << Schedule::BOOLEAN_OPERATOR_GT << " --i iter --i2 10 --o is_finished" << std::endl;
        std::cerr << "    --schedule test --add operator --type " << Schedule::BOOLEAN_OPERATOR_FILE_EXISTS << " --i initial_model --o is_finished" << std::endl;
