...
class project_parameters {
    ...
    void project() {
        ...
        if (nr_uniform > 0) {
            ...
            vol.MDMainHeader.getValue(EMDL::IMAGE_SAMPLINGRATE_X, angpix);
        ...
        if (do_only_one) {
            ...
            FOR_ALL_OBJECTS_IN_METADATA_TABLE(MDang) {
                ...
                MDang.getValue(EMDL::ORIENT_ORIGIN_X_ANGSTROM, xoff);
                MDang.getValue(EMDL::ORIENT_ORIGIN_Y_ANGSTROM, yoff);
                if (do_3d_rot)
                    MDang.getValue(EMDL::ORIENT_ORIGIN_Z_ANGSTROM, zoff);
                ...
                if (do_ctf || do_ctf2) {
                    if (do_3d_rot) {
                        Image<RFLOAT> Ictf;
                        FileName fn_ctf;
                        MDang.getValue(EMDL::CTF_IMAGE, fn_ctf);
                        Ictf.read(fn_ctf);
                ...
                if (do_add_noise) {
                    if (fn_model !="") {
                        ...
                        if (MDang.containsLabel(EMDL::MLMODEL_GROUP_NAME)) {
                            MDang.getValue(EMDL::MLMODEL_GROUP_NAME, fn_group);
                        } else {
                            if (MDang.containsLabel(EMDL::MICROGRAPH_NAME)) {
                                FileName fn_orig, fn_pre, fn_jobnr;
                                MDang.getValue(EMDL::MICROGRAPH_NAME, fn_orig);
                        ...
                        if (MDang.containsLabel(EMDL::IMAGE_NORM_CORRECTION)) {
                            MDang.getValue(EMDL::IMAGE_NORM_CORRECTION, normcorr);
                        }
                ...
                if (do_subtract_exp || do_simulate) {
                    MDang.getValue(EMDL::IMAGE_NAME, fn_expimg);
                ...
                if (do_simulate) {
                    ...
                    if (fn_ang_simulate == "") {
                        ...
                        MDang.getValue(EMDL::ORIENT_ROT, rot, random_imgno);
                        MDang.getValue(EMDL::ORIENT_TILT, tilt, random_imgno);
                        MDang.getValue(EMDL::ORIENT_PSI, psi, random_imgno);
                        MDang.getValue(EMDL::ORIENT_ORIGIN_X_ANGSTROM, xoff, random_imgno);
                        MDang.getValue(EMDL::ORIENT_ORIGIN_Y_ANGSTROM, yoff, random_imgno);
                        if (do_3d_rot)
                            MDang.getValue(EMDL::ORIENT_ORIGIN_Z_ANGSTROM, zoff, random_imgno);

                        xoff /= angpix;
                        yoff /= angpix;
                        zoff /= angpix;
                    } else {
                        MDang_sim.getValue(EMDL::ORIENT_ROT, rot, imgno);
                        MDang_sim.getValue(EMDL::ORIENT_TILT, tilt, imgno);
                        MDang_sim.getValue(EMDL::ORIENT_PSI, psi, imgno);
                        MDang_sim.getValue(EMDL::ORIENT_ORIGIN_X_ANGSTROM, xoff, imgno);
                        MDang_sim.getValue(EMDL::ORIENT_ORIGIN_Y_ANGSTROM, yoff, imgno);
                        if (do_3d_rot)
                            MDang_sim.getValue(EMDL::ORIENT_ORIGIN_Z_ANGSTROM, zoff, imgno);
