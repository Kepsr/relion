...
void CtffindRunner::initialise() {
    ...
    if (!do_phaseshift && (
        additional_gctf_options.find("--phase_shift_L") != std::string::npos ||
    ...
    if (fn_in.isStarFile()) {
        ...
        for (long int i : MDin) {
            FileName fn_mic = MDin.getValue<std::string>(EMDL::MICROGRAPH_NAME, i);

            fn_micrographs_all.push_back(fn_mic);  // Dose-weighted image

            if (do_use_without_doseweighting)
                fn_mic = MDin.getValue<std::string>(EMDL::MICROGRAPH_NAME_WODOSE, i);
            else if (use_given_ps)
                fn_mic = MDin.getValue<std::string>(EMDL::CTF_POWER_SPECTRUM, i);
            fn_micrographs_ctf_all.push_back(fn_mic); // Image for CTF estsimation

            int optics_group = MDin.getValue<int>(EMDL::IMAGE_OPTICS_GROUP, i);
    ...
    if (!obsModel.opticsMdt.containsLabel(EMDL::CTF_CS)) {
        if (Cs < 0.0) {
    ...
    for (long int imic = 0; imic < fn_mic_given_all.size(); imic++) {
        ...
        if (continue_old) {
            ...
            if (getCtffindResults(
                ...
                // false: dont warn if not found Final values
            )) {
...
void CtffindRunner::joinCtffindResults() {
    ...
    for (long int imic = 0; imic < fn_micrographs_all.size(); imic++) {
        ...
        RFLOAT maxres = -999.0, valscore = -999.0, phaseshift = -999.0;
        bool has_this_ctf = getCtffindResults(
            fn_microot, defU, defV, defAng, CC,
...
void CtffindRunner::executeGctf(
    long int imic, std::vector<std::string> &allmicnames, bool is_last, int rank
) {
...
void CtffindRunner::executeCtffind3(long int imic) {
    ...
    // Really? Writing a script at run-time?
...
bool CtffindRunner::getCtffindResults(
    FileName fn_microot, RFLOAT &defU, RFLOAT &defV, RFLOAT &defAng, RFLOAT &CC,
    RFLOAT &HT, RFLOAT &CS, RFLOAT &AmpCnst, RFLOAT &XMAG, RFLOAT &DStep,
    RFLOAT &maxres, RFLOAT &valscore, RFLOAT &phaseshift, bool do_warn
) {
    if (is_ctffind4) {
        return getCtffind4Results(
            fn_microot, defU, defV, defAng, CC, HT, CS, AmpCnst, XMAG, DStep,
            maxres, phaseshift, do_warn
        );
    } else {
        return getCtffind3Results(
            fn_microot, defU, defV, defAng, CC, HT, CS, AmpCnst, XMAG, DStep,
...
bool CtffindRunner::getCtffind3Results(
    FileName fn_microot, RFLOAT &defU, RFLOAT &defV, RFLOAT &defAng, RFLOAT &CC,
    RFLOAT &HT, RFLOAT &CS, RFLOAT &AmpCnst, RFLOAT &XMAG, RFLOAT &DStep,
    ...
    while (getline(in, line, '\n')) {
        ...
        if (
            line.find("CS[mm], HT[kV], AmpCnst, XMAG, DStep[um]") != std::string::npos ||
        ...
        int nr_exp_cols = 6 + do_phaseshift;
...
bool CtffindRunner::getCtffind4Results(
    FileName fn_microot, RFLOAT &defU, RFLOAT &defV, RFLOAT &defAng, RFLOAT &CC,
    RFLOAT &HT, RFLOAT &CS, RFLOAT &AmpCnst, RFLOAT &XMAG, RFLOAT &DStep,
